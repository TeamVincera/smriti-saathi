...
var RETICLE_HYDRATION_SIGNAL = "reticle:hydration-complete";
var RETICLE_RENDERS_STORE = "__reticle_renders";
var RETICLE_RENDER_PREHOOK = "__reticleRenderPreHook";
var RETICLE_ERROR_BOUNDARY_SIGNAL = "reticle:error-boundary";
var RETICLE_HYDRATION_ERROR_SIGNAL = "reticle:hydration-error";
...
function buildRedactionPolicy(config, onWarn) {
  var _a3;
  const literalKeys = normalizeNames((_a3 = config == null ? void 0 : config.keys) == null ? void 0 : _a3.filter((k) => "string" === typeof k));
  const patterns = ((config == null ? void 0 : config.keys) ?? []).filter((k) => k instanceof RegExp);
  const allowed = normalizeNames(config == null ? void 0 : config.allow);
...
function wireRedactionKeys(config) {
  ...
  const out = [];
  for (const key of (config == null ? void 0 : config.keys) ?? []) {
...
// node_modules/@reticlehq/core/dist/event-priority.js
...
// node_modules/@reticlehq/core/dist/findings.js
var ContradictionKind = {
...
var ABSENCE_DERIVED_CONTRADICTIONS = /* @__PURE__ */ new Set([
...
// node_modules/@reticlehq/core/dist/consequence.js
var ConsequenceKind = {
  SIGNAL: "signal",
  NET: "net",
  STATE: "state"
...
var CONSEQUENCE_KINDS = new Set(Object.values(ConsequenceKind));
var PRESENCE_KINDS = new Set(Object.values(PresenceKind));
...
// node_modules/@reticlehq/core/dist/journal.js
var JOURNAL_FILE_VERSION = 1;
var JournalActionSchema = external_exports.object({
  v: external_exports.literal(JOURNAL_FILE_VERSION),
  /** The command correlation id (`c<n>`) — the natural, cross-side-stable action identity. */
  actionId: external_exports.string().min(1).max(TRANSPORT_LIMITS.MAX_REF_LENGTH),
  /** The MCP tool / command name that drove it (e.g. "reticle_act"). */
  tool: external_exports.string().min(1).max(TRANSPORT_LIMITS.MAX_COMMAND_NAME_LENGTH),
  /** Redacted invocation args. */
  args: external_exports.record(external_exports.unknown()).default({}),
  /** Bounded effect summary (settle glyph, target, etc.). Kept open; narrowed by the writer. */
  effect: external_exports.unknown().optional(),
  /** Whether the page settled within budget after the action, when known. */
  settled: external_exports.boolean().optional(),
  /** Time from dispatch to settle in ms, when known. */
  settledInMs: external_exports.number().int().min(0).optional(),
  /** Inclusive seq range of events attributed to this action, when any were observed. */
  seqRange: external_exports.object({ from: external_exports.number().int().min(0), to: external_exports.number().int().min(0) }).optional(),
  /** Elapsed-ms window [dispatch, resolve] used for window attribution. */
  tRange: external_exports.object({ from: external_exports.number().int().min(0), to: external_exports.number().int().min(0) }),
  /** Elapsed-ms timestamp the action was recorded at (clock injected by the writer). */
  at: external_exports.number().int().min(0)
});
var JournalVerdictEffectSchema = external_exports.object({
  /** What was claimed, in the caller's own words — the subject a later proof supersedes. */
  claim: external_exports.string().min(1).max(TRANSPORT_LIMITS.MAX_STRING_LENGTH),
  /** The verdict, as the one field an agent reads. The enum, never a re-typed copy of it. */
  verified: external_exports.nativeEnum(Verified),
  /** `file:line` for the element driven, when the page told us one. */
  source: external_exports.string().max(TRANSPORT_LIMITS.MAX_URL_LENGTH).optional()
});

// node_modules/@reticlehq/core/dist/daemon-registry.js
var DaemonRegistryEntrySchema = external_exports.object({
...
// node_modules/@reticlehq/core/dist/project-registry.js
var ProjectRegistryEntrySchema = external_exports.object({
...
var ProjectRegistrySchema = external_exports.object({
  version: external_exports.literal(1),
  projects: external_exports.record(external_exports.string(), ProjectRegistryEntrySchema)
});

// node_modules/@reticlehq/core/dist/intent.js
var INTENT_FILE_VERSION = 1;
var IntentState = {
...
var IntentSurfaceSchema = external_exports.object({
  route: external_exports.string().optional(),
  flow: external_exports.string().optional(),
  files: external_exports.array(external_exports.string()).optional()
});
var IntentProofSchema = external_exports.object({
  verdictId: external_exports.string(),
...
var IntentSchema = external_exports.object({
  ...
  state: external_exports.enum([IntentState.DECLARED, IntentState.BOUND, IntentState.PROVED]),
  declaredAt: external_exports.number(),
  /** Left as `unknown`: core must not depend on the server's predicate vocabulary. */
  binding: external_exports.unknown().optional(),
  ...
  amended: external_exports.array(external_exports.object({ statement: external_exports.string(), at: external_exports.number() })).optional()
});
var IntentFileSchema = external_exports.object({
  version: external_exports.literal(INTENT_FILE_VERSION),
  intents: external_exports.record(external_exports.string(), IntentSchema)
});

// node_modules/@reticlehq/core/dist/instrumentation-gap.js
var InstrumentationGapKind = {
  /**
...
var GAP_FIX = {
  [InstrumentationGapKind.NO_SOURCE_MAPPING]: "add the Reticle build plugin (@reticlehq/vite-plugin, @reticlehq/next, or the babel plugin) so elements carry data-reticle-source",
  [InstrumentationGapKind.NO_STORE_REGISTERED]: "register the store with reticle.registerStore(name, getState) so state can be read directly instead of inferred from the DOM",
  [InstrumentationGapKind.NO_SIGNAL_ON_MUTATION]: "fire reticle.signal(name, data) where this state is committed — commitAndSignal binds the two so they cannot drift",
  [InstrumentationGapKind.NO_ROUTE_SIGNAL]: "let the Reticle router adapter observe navigation, or fire reticle.signal on route commit",
  [InstrumentationGapKind.UNDECLARED_CONTROL]: "add this control to reticle.describe() so the capability contract matches what the app actually exposes",
  [InstrumentationGapKind.MISSING_TESTID]: 'add data-testid="..." on this control so the flow can name it after a refactor',
  [InstrumentationGapKind.UNDECLARED_CHANGE]: 'declare it with reticle_intent { action: "declare", intents: [{ id, statement }] } — the statement is prose, in your own words: which user does what, and what should become true',
  [InstrumentationGapKind.INTENT_UNDISCHARGED]: 'draw a verdict whose `until` asserts the intent itself, then it discharges — or call reticle_run({ tool: "reticle_context" }) to see exactly what is still owed',
  [InstrumentationGapKind.NO_FLOW_INTENT]: 'save it again with intent: "<which user does what, and what should become true>" — prose, in your own words — or set intentId to an intent already in the ledger'
};

// node_modules/@reticlehq/core/dist/security.js
var DANGEROUS_ACTION = /\b(delete|remove|destroy|erase|drop|terminate|revoke|reset|logout|log out|sign out|close account|cancel subscription|purchase|buy|pay|payment|place order|confirm order|send money|send funds|transfer|withdraw|refund)\b/i;
...
// node_modules/@reticlehq/core/dist/toon.js
var STATE_FLAG = {
...
var UNENCODED_STATES = /* @__PURE__ */ new Set([
  ElementState.PRESENT,
  ElementState.VISIBLE,
  ElementState.HIDDEN
]);

// node_modules/@reticlehq/core/dist/upgrade.js
var CloudCapability = {
...
var UpgradeHintSchema = external_exports.object({
  capability: external_exports.nativeEnum(CloudCapability),
  reason: external_exports.string(),
  unlockedBy: external_exports.string(),
  learnMoreUrl: external_exports.string().url()
});
var COPY_BY_CAPABILITY = {
  ...
  [CloudCapability.HOSTED_RUNNER]: {
    reason: "Verifying without a local browser (CI, a background agent, or a deployed URL) needs a browser we run for you.",
    unlockedBy: "Link a Reticle Cloud account to run verifications on hosted browsers."
  }
};

// node_modules/@reticlehq/core/dist/telemetry-session.js
var ErrorShapeSchema = external_exports.object({
  /** Stable group key — the same defect from any machine shares it. */
  fingerprint: external_exports.string().min(1).max(64),
...
var MachineSnapshotSchema = external_exports.object({
  /** Resident set size of the daemon itself — our footprint, the one we can actually fix. */
  rssMb: external_exports.number().int().nonnegative(),
  heapUsedMb: external_exports.number().int().nonnegative(),
  /** Free and total system memory. A machine at 200 MB free explains a lot of otherwise-weird bugs. */
  freeMemMb: external_exports.number().int().nonnegative(),
  totalMemMb: external_exports.number().int().nonnegative(),
  /** 1-minute load average, ×100 so it stays an integer. Zero on Windows, which does not report it. */
  load1x100: external_exports.number().int().nonnegative(),
  cpuCount: external_exports.number().int().nonnegative()
});
var ToolTimingSchema = external_exports.object({
  ...
  maxMs: external_exports.number().int().nonnegative()
});
var ConnectionStatsSchema = external_exports.object({
  attempts: external_exports.number().int().nonnegative(),
  successes: external_exports.number().int().nonnegative(),
  /** Failure reasons with counts — our own classified vocabulary, never a raw error string. */
  failures: external_exports.record(external_exports.string(), external_exports.number().int().nonnegative()).optional()
});
var SessionSummarySchema = external_exports.object({
   ...
   */
  toolCounts: external_exports.record(external_exports.string(), external_exports.number().int().nonnegative()),
  /** Tool calls that ended in an error, and the distinct error shapes behind them. */
  toolErrors: external_exports.number().int().nonnegative(),
  /**
   ...
   */
  errors: external_exports.array(ErrorShapeSchema).max(40).optional(),
  /**
   ...
   */
  sdkFailures: external_exports.number().int().nonnegative().optional(),
  /** The distinct SDK failure shapes, with the module that reported each. */
  sdkErrors: external_exports.array(ErrorShapeSchema).max(40).optional(),
  /** Verifications that produced a verdict during the session. */
  verifications: external_exports.number().int().nonnegative(),
  /** Defects found in the app under test — the outcome number, not an activity number. */
  bugsFound: external_exports.number().int().nonnegative().optional(),
  /** Those bugs by kind, so the headline number can always be broken down rather than asserted. */
  bugKinds: external_exports.record(external_exports.string(), external_exports.number().int().nonnegative()).optional(),
  /**
   ...
   */
  toolParams: external_exports.record(external_exports.string(), external_exports.record(external_exports.string(), external_exports.number().int().nonnegative())).optional(),
  /**
   ...
   */
  connections: external_exports.record(external_exports.string(), ConnectionStatsSchema).optional(),
  /** Wall-clock per tool. With `toolCounts` this gives average, worst, and total time in Reticle. */
  toolTiming: external_exports.record(external_exports.string(), ToolTimingSchema).optional(),
  /**
   ...
   */
  busyMs: external_exports.number().int().nonnegative().optional(),
  /**
   ...
   */
  browserMs: external_exports.number().int().nonnegative().optional(),
  browserCommands: external_exports.number().int().nonnegative().optional(),
  /** The most tool calls in flight at once — parallel agents, or one agent fanning out. */
  peakConcurrentTools: external_exports.number().int().nonnegative().optional(),
  /** Calls for a tool name that does not exist. A non-zero value means our surface is confusing. */
  unknownToolCalls: external_exports.number().int().nonnegative().optional(),
  /**
   ...
   */
  unknownTools: external_exports.record(external_exports.string().max(64), external_exports.number().int().nonnegative()).optional(),
  /**
   ...
   */
  noSessionErrors: external_exports.number().int().nonnegative().optional(),
  /**
   ...
   */
  consecutiveRepeats: external_exports.record(external_exports.number().int().positive()).optional(),
  /**
   ...
   */
  abandonedActions: external_exports.number().int().nonnegative().optional(),
  ...
  clients: external_exports.array(external_exports.string().max(64)).max(8).optional(),
  /**
   ...
   */
  clientVersions: external_exports.record(external_exports.string().max(64), external_exports.string().max(32)).optional(),
  /**
   ...
   */
  surface: external_exports.string().max(32).optional(),
  /**
   ...
   */
  appConnects: external_exports.number().int().nonnegative().optional(),
  /**
   ...
   */
  msToFirstApp: external_exports.number().int().nonnegative().optional(),
  /**
   ...
   */
  endReason: external_exports.enum(["never_used", "explored", "abandoned", "verified", "client_left"]).optional(),
  /**
   ...
   * rather than a hope.
   */
  feedbackPrompted: external_exports.number().int().nonnegative().optional(),
  /**
   ...
   */
  errorClasses: external_exports.record(external_exports.string().max(16), external_exports.number().int().nonnegative()).optional(),
  /**
   ...
   */
  errorsRecovered: external_exports.number().int().nonnegative().optional(),
  /** Errors whose very next call failed again — the message did not land. */
  errorsRepeated: external_exports.number().int().nonnegative().optional(),
  /**
   ...
   */
  endedWithVerdict: external_exports.boolean().optional(),
   ...
   */
  exit: external_exports.enum(["idle", "signal", "unknown"]).optional()
});
var ProjectSize = {
  TINY: "tiny",
  // < 50 source files — a demo or a spike
  SMALL: "small",
  // < 250
  MEDIUM: "medium",
  // < 1000
  ...
  // < 5000
  HUGE: "huge"
  // 5000+ — a monorepo or a mature product
};
var ProjectProfileSchema = external_exports.object({
   ...
   */
  ageWeeks: external_exports.number().int().nonnegative().optional(),
  /** Saved flows on disk — the deterministic-replay suite. 0 means replay was never adopted. */
  flowCount: external_exports.number().int().nonnegative(),
  ...
  visualBaselineCount: external_exports.number().int().nonnegative(),
  ...
  hasContract: external_exports.boolean(),
  /** Fail-to-pass bug capsules — the deepest feature in the product. */
  capsuleCount: external_exports.number().int().nonnegative(),
  /** Which of Reticle's feature FAMILIES this project has ever touched. The activation metric. */
  featuresUsed: external_exports.array(external_exports.string().max(32)).max(24),
  /** 0–1: the fraction of shipped feature families in use. One number for "are they getting value". */
  featureDepth: external_exports.number().min(0).max(1)
});

// node_modules/@reticlehq/core/dist/telemetry-license.js
...
// node_modules/@reticlehq/core/dist/telemetry.js
var TELEMETRY_EVENT_VERSION = 3;
...
var SESSION_SCOPED = /* @__PURE__ */ new Set([
...
var VerificationSchema = external_exports.object({
  ...
  via: external_exports.string().min(1).max(64),
  /** `yes` | `no` | `unknown` — the honest verdict, where `unknown` means the evidence could not decide. */
  verified: external_exports.string().min(1).max(16),
   ...
   */
  falseGreenCaught: external_exports.boolean(),
   ...
   */
  browser: external_exports.string().min(1).max(16).optional(),
  /**
   ...
   */
  reason: external_exports.nativeEnum(VerifiedReason).optional(),
  /**
   ...
   */
  uncleanLoss: external_exports.nativeEnum(CaptureLoss).optional()
});
var VersionChangeSchema = external_exports.object({
  from: external_exports.string().min(1).max(64),
   ...
   */
  nudged: external_exports.boolean().optional(),
  /** `update` (forward) or `rollback` (back) — a rollback is a release-quality alarm. */
  direction: external_exports.string().min(1).max(16)
});
var CrashPort = {
  RETICLE: "reticle",
  OTHER: "other"
};
var CrashSchema = external_exports.object({
  ...
  errorType: external_exports.string().min(1).max(64).optional(),
  /** Hash of the type + our own frames. Groups the same crash across every machine. */
  fingerprint: external_exports.string().min(1).max(64).optional(),
  /**
   ...
   */
  frames: external_exports.array(external_exports.string().max(120)).max(12).optional(),
  /** The MCP tool in flight when it happened, when there was one. The trigger point. */
  tool: external_exports.string().min(1).max(64).optional(),
  /**
   ...
   */
  breadcrumb: external_exports.array(external_exports.string().max(64)).max(12).optional(),
  /** Node major/minor — crashes cluster hard by runtime version and we were not recording it. */
  nodeVersion: external_exports.string().max(24).optional(),
  /** `arm64` / `x64`. A surprising number of native-module failures are architecture-specific. */
  arch: external_exports.string().max(16).optional(),
  /**
   ...
   */
  syscall: external_exports.string().max(32).optional(),
  /**
   * The symbolic errno — `ECONNREFUSED`, `EPIPE`. The name, never the platform-specific number.
   */
  errno: external_exports.string().max(32).optional(),
  /**
   ...
   */
  loopback: external_exports.boolean().optional(),
  /** Whether the port was one of ours, as an enum. The number itself is never sent. */
  port: external_exports.enum([CrashPort.RETICLE, CrashPort.OTHER]).optional(),
  /**
   ...
   */
  internalFrame: external_exports.string().max(64).optional()
});
var BugSource = {
...
var BugFoundSchema = external_exports.object({
  source: external_exports.nativeEnum(BugSource),
   ...
   */
  falseGreen: external_exports.boolean(),
   ...
   */
  attribution: external_exports.nativeEnum(BugAttribution),
  /**
   ...
   */
  fingerprint: external_exports.string().max(16).optional()
});
var RefusalReason = {
...
var McpConnectionSchema = external_exports.object({
  /** False the first time a client attaches to this daemon; true for every attach after. */
  reconnect: external_exports.boolean(),
  /**
   ...
   */
  daemonAgeMs: external_exports.number().int().nonnegative(),
  /** Which client, from its own handshake (`claude-code`, `cursor`). */
  client: external_exports.string().min(1).max(64).optional()
});
var AppInstrumentationSchema = external_exports.object({
  ...
  agentAttached: external_exports.boolean(),
  /** How long the daemon had been up. Large values mean Reticle sat there with nothing wired. */
  msToFirstApp: external_exports.number().int().nonnegative()
});
var OutageStage = {
  FIRST: "first",
...
var McpOutageSchema = external_exports.object({
  stage: external_exports.nativeEnum(OutageStage),
  reason: external_exports.nativeEnum(OutageReason),
   ...
   * worth driving down.
   */
  pendingLost: external_exports.number().int().nonnegative().optional()
});
var InstallSource = {
...
var TelemetryEventSchema = external_exports.object({
  ...
  event: external_exports.nativeEnum(TelemetryEventKind),
   ...
   */
  automation: external_exports.nativeEnum(AutomationHint).optional(),
  /**
   ...
   */
  projectIdSource: external_exports.nativeEnum(ProjectIdSource).optional(),
  /** `process.platform` (darwin/linux/win32) — OS mix, low cardinality, non-identifying. */
  os: external_exports.string().min(1).max(32),
  /**
   ...
   */
  tzOffsetMin: external_exports.number().int().min(-900).max(900).optional(),
  ...
  actor: external_exports.nativeEnum(TelemetryActor).optional(),
  /** Only on `cli_command_run`: which subcommand. */
  command: external_exports.string().min(1).max(32).optional(),
  /**
   ...
   */
  flags: external_exports.array(external_exports.string().max(32)).max(16).optional(),
   ...
   */
  installSource: external_exports.nativeEnum(InstallSource).optional(),
  /**
   ...
   */
  licenseId: external_exports.string().min(1).max(64).optional(),
  /** How activation resolved. Absent on a build with no issuer key baked, i.e. every OSS install. */
  licenseStatus: external_exports.nativeEnum(LicenseActivation).optional()
});
...
function readBase(store) {
  try {
    const raw = store == null ? void 0 : store.getItem(REF_BASE_KEY);
...
var _toRef, _fromRef, _storage, _seq, _reserved, _mintsSinceSweep, _editedAtSeq, _RefRegistry_instances, reserve_fn, evictOldest_fn, sweep_fn;
var RefRegistry = class {
  ...
  constructor(storage = () => globalThis.sessionStorage) {
    __privateAdd(this, _RefRegistry_instances);
    __privateAdd(this, _toRef, /* @__PURE__ */ new WeakMap());
    __privateAdd(this, _fromRef, /* @__PURE__ */ new Map());
    ...
    __privateAdd(this, _reserved, 0);
    __privateAdd(this, _mintsSinceSweep, 0);
    /**
   ...
   */
  markEdited() {
    __privateSet(this, _editedAtSeq, __privateGet(this, _seq));
  }
  /** Was this ref handed out before the last hot update? False while no update has been observed. */
  mintedBeforeLastEdit(ref) {
    if (0 === __privateGet(this, _editedAtSeq) || !ref.startsWith(REF_PREFIX))
      return false;
    const n = Number(ref.slice(REF_PREFIX.length));
    return Number.isSafeInteger(n) && n > 0 && n <= __privateGet(this, _editedAtSeq);
  }
  /** Get the existing ref for an element, or mint a new one. */
  refFor(el) {
    const existing = __privateGet(this, _toRef).get(el);
    if (existing !== void 0) {
      const weak = __privateGet(this, _fromRef).get(existing);
      __privateGet(this, _fromRef).delete(existing);
      __privateGet(this, _fromRef).set(existing, weak ?? new WeakRef(el));
      return existing;
    }
    if (__privateGet(this, _seq) >= __privateGet(this, _reserved))
      __privateMethod(this, _RefRegistry_instances, reserve_fn).call(this);
    __privateSet(this, _seq, __privateGet(this, _seq) + 1);
    const ref = asRef(`${REF_PREFIX}${String(__privateGet(this, _seq))}`);
    __privateGet(this, _toRef).set(el, ref);
    __privateGet(this, _fromRef).set(ref, new WeakRef(el));
    __privateSet(this, _mintsSinceSweep, __privateGet(this, _mintsSinceSweep) + 1);
    if (__privateGet(this, _mintsSinceSweep) >= SWEEP_EVERY_MINTS) {
      __privateSet(this, _mintsSinceSweep, 0);
      __privateMethod(this, _RefRegistry_instances, sweep_fn).call(this);
    } else if (__privateGet(this, _fromRef).size > MAX_TRACKED_REFS) {
      __privateMethod(this, _RefRegistry_instances, evictOldest_fn).call(this);
    }
    return ref;
  }
  /**
   ...
   */
  resolve(ref) {
    const weak = __privateGet(this, _fromRef).get(ref);
    if (weak === void 0)
      return null;
    const el = weak.deref();
    if (el === void 0 || !el.isConnected) {
      __privateGet(this, _fromRef).delete(ref);
      return null;
    }
    __privateGet(this, _fromRef).delete(ref);
    __privateGet(this, _fromRef).set(ref, weak);
    return el;
  }
};
_toRef = new WeakMap();
_fromRef = new WeakMap();
_storage = new WeakMap();
_seq = new WeakMap();
_reserved = new WeakMap();
_mintsSinceSweep = new WeakMap();
_editedAtSeq = new WeakMap();
_RefRegistry_instances = new WeakSet();
/** Read the tab's high-water mark and claim the next block, BEFORE any of it is handed out. */
reserve_fn = function() {
  let store;
  try {
    store = __privateGet(this, _storage).call(this);
  } catch {
    store = void 0;
  }
  const base = Math.max(readBase(store), __privateGet(this, _seq));
  __privateSet(this, _reserved, base + REF_BLOCK);
  try {
    store == null ? void 0 : store.setItem(REF_BASE_KEY, String(__privateGet(this, _reserved)));
  } catch {
  }
  __privateSet(this, _seq, base);
};
...
evictOldest_fn = function() {
  const oldest = __privateGet(this, _fromRef).keys().next();
  if (oldest.done !== true)
    __privateGet(this, _fromRef).delete(oldest.value);
};
...
sweep_fn = function() {
  for (const [ref, weak] of __privateGet(this, _fromRef)) {
    const el = weak.deref();
    if (el === void 0 || !el.isConnected)
      __privateGet(this, _fromRef).delete(ref);
  }
  while (__privateGet(this, _fromRef).size > MAX_TRACKED_REFS)
    __privateMethod(this, _RefRegistry_instances, evictOldest_fn).call(this);
};
...
function canvasChartData(canvas, globals) {
  var _a3, _b, _c, _d, _e;
  const chartjs = (_b = (_a3 = globals.Chart) == null ? void 0 : _a3.getChart) == null ? void 0 : _b.call(_a3, canvas);
  if (chartjs !== void 0 && chartjs !== null) {
    return { library: "chartjs", data: chartjs.data ?? null };
  }
  const echart = (_d = (_c = globals.echarts) == null ? void 0 : _c.getInstanceByDom) == null ? void 0 : _d.call(_c, canvas);
  if (echart !== void 0 && echart !== null) {
    return { library: "echarts", data: ((_e = echart.getOption) == null ? void 0 : _e.call(echart)) ?? null };
  }
  return null;
}
...
function sanitize(value, state, depth2, key) {
  ...
  if (null === value || "boolean" === typeof value)
    return value;
  if ("string" === typeof value) {
    return boundedString(value, state, "error" === (key == null ? void 0 : key.toLowerCase()) ? TRANSPORT_LIMITS.MAX_ERROR_LENGTH : TRANSPORT_LIMITS.MAX_STRING_LENGTH);
  }
...
function isVisible(el, memo) {
  if (!el.isConnected)
    return false;
  const cached = memo == null ? void 0 : memo.get(el);
  if (cached !== void 0)
    return cached;
...
function isIgnored(el) {
  const sel3 = extraIgnore.length > 0 ? `${RETICLE_OVERLAY},${DEV_OVERLAYS},${extraIgnore}` : `${RETICLE_OVERLAY},${DEV_OVERLAYS}`;
...
function pierceChildren(parent) {
  ...
  if (isFrame(parent)) {
    try {
      const body = (_a3 = parent.contentDocument) == null ? void 0 : _a3.body;
...
var capabilities = globalStore.__reticleCapabilities ?? (globalStore.__reticleCapabilities = empty());
...
var adapters = globalStore2.__reticleAdapters ?? (globalStore2.__reticleAdapters = []);
...
var COMPONENT_CANDIDATE_SELECTOR = `[${SOURCE_ATTR2}], [${TESTID_ATTR}], button, a, input, select, textarea, [role]`;
...
function readableFrameBody(frame) {
  var _a3;
  try {
    return ((_a3 = frame.contentDocument) == null ? void 0 : _a3.body) ?? null;
  } catch {
    return null;
  }
}
...
function resolveLabelledBy(el) {
  ...
  const text = ids.split(/\s+/).map((id) => {
    var _a3, _b;
    return id.length > 0 ? ((_b = (_a3 = el.ownerDocument.getElementById(id)) == null ? void 0 : _a3.textContent) == null ? void 0 : _b.trim()) ?? "" : "";
  ...
  for (const role of CONTAINER_ROLES) {
    ...
    for (const el of containers) {
      ...
      for (const child of Array.from(children)) {
        ...
        const childName = child.getAttribute("aria-label") ?? child.getAttribute("data-testid") ?? ((_a3 = child.textContent) == null ? void 0 : _a3.trim().slice(0, 40)) ?? "";
...
var g = globalThis;
var realSetTimeout = "function" === typeof g.setTimeout ? g.setTimeout.bind(g) : null;
var realClearTimeout = "function" === typeof g.clearTimeout ? g.clearTimeout.bind(g) : null;
var realRaf = "function" === typeof g.requestAnimationFrame ? g.requestAnimationFrame.bind(g) : null;
var _a;
var realPerfNow = "function" === typeof ((_a = g.performance) == null ? void 0 : _a.now) ? g.performance.now.bind(g.performance) : null;
...
var nativeClearTimeout = (id) => {
  realClearTimeout == null ? void 0 : realClearTimeout(id);
};
...
var JOIN = " | ";
var TEXT_NODE = 3;
var HAS_LETTER = new RegExp("\\p{L}", "u");
function saysSomething(text) {
  return HAS_LETTER.test(text);
}
var _seen, _length, _AppearedText_instances, full_fn, add_fn;
var AppearedText = class {
  constructor() {
    __privateAdd(this, _AppearedText_instances);
    __privateAdd(this, _seen, /* @__PURE__ */ new Set());
    __privateAdd(this, _length, 0);
  }
  collect(records) {
    for (const record3 of records) {
      if (__privateMethod(this, _AppearedText_instances, full_fn).call(this))
        return;
      if ("characterData" === record3.type) {
        __privateMethod(this, _AppearedText_instances, add_fn).call(this, record3.target.textContent, record3.target.parentElement);
        continue;
      }
      for (const node of record3.addedNodes) {
        ...
        __privateMethod(this, _AppearedText_instances, add_fn).call(this, node.textContent, owner);
      }
    }
  }
  /**
   ...
   */
  effect(wrote) {
    const said = [...__privateGet(this, _seen)].filter((text) => text !== wrote);
    if (0 === said.length)
      return {};
...
_seen = new WeakMap();
_length = new WeakMap();
_AppearedText_instances = new WeakSet();
full_fn = function() {
  return __privateGet(this, _length) > APPEARED_MAX;
};
add_fn = function(raw, owner) {
  ...
  if (!saysSomething(text))
    return;
  if (__privateGet(this, _seen).has(text))
    return;
  __privateGet(this, _seen).add(text);
  __privateSet(this, _length, __privateGet(this, _length) + (text.length + JOIN.length));
};
...
function anchorOf(el) {
  ...
  if (testid !== void 0)
    out.testid = testid;
  const component = info == null ? void 0 : info.componentStack[0];
  if (component !== void 0)
    out.component = component;
  if ((info == null ? void 0 : info.source) !== void 0)
...
function dangerousActionContext(el) {
  ...
  return [
    ...
    (form == null ? void 0 : form.getAttribute("action")) ?? ""
  ].join(" ");
}
...
function assertActionAllowed(el, action, args) {
  ...
  const context = isHtmlElement(dragTarget) ? `${dangerousActionContext(el)} ${dangerousActionContext(dragTarget)}` : dangerousActionContext(el);
...
var g2 = globalThis;
var _a2;
var realWarn = "function" === typeof ((_a2 = g2.console) == null ? void 0 : _a2.warn) ? g2.console.warn.bind(g2.console) : null;
var nativeWarn = (...args) => {
  realWarn == null ? void 0 : realWarn(...args);
};

// node_modules/@reticlehq/browser/dist/registry/stores.js
var globalStore3 = globalThis;
var stores = globalStore3.__reticleStores ?? (globalStore3.__reticleStores = /* @__PURE__ */ new Map());
var subscribers = globalStore3.__reticleStoreSubs ?? (globalStore3.__reticleStoreSubs = /* @__PURE__ */ new Map());
var registrationListeners = globalStore3.__reticleStoreListeners ?? (globalStore3.__reticleStoreListeners = /* @__PURE__ */ new Set());
...
function resolveMarkAnchor(el) {
  const testid = el.getAttribute(TESTID_ATTR2) ?? void 0;
  const info = identifyComponent(el);
  const component = info == null ? void 0 : info.componentStack[0];
  const source = sourceFor(el, info == null ? void 0 : info.source);
  const role = getRole(el);
  const name = getAccessibleName(el);
  const input = {};
  if (testid !== void 0)
...
function appendLogRow(container, kind, text, ts, logMax) {
  ...
  while (container.childElementCount > logMax)
    (_a3 = container.firstElementChild) == null ? void 0 : _a3.remove();
  requestAnimationFrame(() => {
    scrollLogToLatest(container);
  });
  return {
...
function effectivePaceMs(requested, nav = "undefined" === typeof navigator ? void 0 : navigator) {
  if (requested !== void 0)
    return requested;
  return true === (nav == null ? void 0 : nav.webdriver) ? 0 : DEFAULT_PACE;
}
var GlowPhase = {
  IDLE: "idle",
  BUSY: "busy",
...
function unionRects(rects) {
  ...
  if (0 === rects.length)
    return void 0;
  let left = ((_a3 = rects[0]) == null ? void 0 : _a3.left) ?? 0;
  let top = ((_b = rects[0]) == null ? void 0 : _b.top) ?? 0;
  let right = ((_c = rects[0]) == null ? void 0 : _c.right) ?? 0;
  let bottom = ((_d = rects[0]) == null ? void 0 : _d.bottom) ?? 0;
...
function installHudPositionGuards(hud, overlay) {
  ...
  (_a3 = window.visualViewport) == null ? void 0 : _a3.addEventListener("resize", onVisualViewportResize, { signal });
  (_b = window.visualViewport) == null ? void 0 : _b.addEventListener("scroll", onVisualViewportResize, { signal });
  return () => {
    listeners2.abort();
...
function installHudDrag(hud, head, callbacks = {}) {
  ...
  const finishDrag = () => {
    ...
    (_a3 = callbacks.onDragEnd) == null ? void 0 : _a3.call(callbacks, didMove);
  };
  ...
  const onPointerMove = (e) => {
    ...
    if (!moved && Math.abs(dx) < HUD_DRAG_THRESHOLD_PX && Math.abs(dy) < HUD_DRAG_THRESHOLD_PX)
      return;
    moved = true;
    (_a3 = callbacks.onDragMove) == null ? void 0 : _a3.call(callbacks);
...
var _root, _panel, _btn, _host, _PresenterSettingsPanel_instances, syncDockLayout_fn, update_fn, syncUi_fn, paintToggle_fn, paintCheck_fn, buildThemes_fn;
var PresenterSettingsPanel = class {
  constructor(host = {}) {
    __privateAdd(this, _PresenterSettingsPanel_instances);
    __privateAdd(this, _root);
    __privateAdd(this, _panel);
    __privateAdd(this, _btn);
  ...
  contains(node) {
    var _a3;
    return true === ((_a3 = __privateGet(this, _panel)) == null ? void 0 : _a3.contains(node));
  }
  mount(root) {
    var _a3, _b, _c, _d, _e, _f;
    __privateSet(this, _root, root);
    const panel = root.querySelector(`[${SETTINGS_PANEL_ATTR}]`);
    __privateSet(this, _panel, panel instanceof HTMLElement ? panel : void 0);
    (_a3 = __privateGet(this, _panel)) == null ? void 0 : _a3.addEventListener("pointerdown", (e) => {
      e.stopPropagation();
    });
    const btn = root.querySelector(`[${SETTINGS_BTN_ATTR}]`);
    __privateSet(this, _btn, btn instanceof HTMLElement ? btn : void 0);
    ...
    if (__privateGet(this, _btn) !== void 0) {
      setHiIcon(__privateGet(this, _btn), PresenterIcon.GEAR, PRESENTER_ICON_SIZE.TOOLBAR);
      __privateGet(this, _btn).addEventListener("click", (e) => {
        e.stopPropagation();
        this.toggle();
      });
    }
    (_b = root.querySelector(`[data-reticle-settings-cycle="outputDetail"]`)) == null ? void 0 : _b.addEventListener("click", (e) => {
      e.stopPropagation();
      ...
      if (next !== void 0)
        __privateMethod(this, _PresenterSettingsPanel_instances, update_fn).call(this, { outputDetail: next.value });
    });
    ...
    for (const toggle of root.querySelectorAll(`[${SETTING_KEY_ATTR}]`)) {
      const activateToggle = () => {
        ...
        if ("reactComponents" === key) {
          __privateMethod(this, _PresenterSettingsPanel_instances, update_fn).call(this, { reactComponents: !activeSettings.reactComponents });
        } else if ("hideUntilRestart" === key) {
          const next = !activeSettings.hideUntilRestart;
          __privateMethod(this, _PresenterSettingsPanel_instances, update_fn).call(this, { hideUntilRestart: next });
          if (next)
            (_b2 = (_a4 = __privateGet(this, _host)).onHideUntilRestart) == null ? void 0 : _b2.call(_a4);
        } else if ("showTally" === key) {
          __privateMethod(this, _PresenterSettingsPanel_instances, update_fn).call(this, { showTally: !activeSettings.showTally });
        } else if ("autoOpenChat" === key) {
          __privateMethod(this, _PresenterSettingsPanel_instances, update_fn).call(this, { autoOpenChat: !activeSettings.autoOpenChat });
        } else if ("showTimestamps" === key) {
          __privateMethod(this, _PresenterSettingsPanel_instances, update_fn).call(this, { showTimestamps: !activeSettings.showTimestamps });
        } else if ("reduceMotion" === key) {
          __privateMethod(this, _PresenterSettingsPanel_instances, update_fn).call(this, { reduceMotion: !activeSettings.reduceMotion });
        } else if ("ambientGlow" === key) {
          __privateMethod(this, _PresenterSettingsPanel_instances, update_fn).call(this, { ambientGlow: !activeSettings.ambientGlow });
        }
      };
    ...
    for (const check of root.querySelectorAll("[data-reticle-check]")) {
      const activate = () => {
        ...
        if ("clearOnCopy" === key) {
          __privateMethod(this, _PresenterSettingsPanel_instances, update_fn).call(this, { clearOnCopy: !activeSettings.clearOnCopy });
        } else if ("blockPageInteractions" === key) {
          __privateMethod(this, _PresenterSettingsPanel_instances, update_fn).call(this, { blockPageInteractions: !activeSettings.blockPageInteractions });
        }
      };
      const row = check.closest("[data-reticle-check-row]");
      row == null ? void 0 : row.addEventListener("click", (e) => {
        e.stopPropagation();
    ...
    (_c = root.querySelector("[data-reticle-settings-mcp]")) == null ? void 0 : _c.addEventListener("click", (e) => {
      e.stopPropagation();
      window.open(MCP_DOCS_URL, "_blank", "noopener,noreferrer");
    });
    (_d = root.querySelector("[data-reticle-settings-reset]")) == null ? void 0 : _d.addEventListener("click", (e) => {
      e.stopPropagation();
    ...
    __privateMethod(this, _PresenterSettingsPanel_instances, buildThemes_fn).call(this);
    __privateMethod(this, _PresenterSettingsPanel_instances, syncUi_fn).call(this);
    applyPresenterSettings(root, activeSettings);
    (_f = (_e = __privateGet(this, _host)).onSettingsChange) == null ? void 0 : _f.call(_e, activeSettings);
  }
  teardown() {
    __privateSet(this, _root, void 0);
    __privateSet(this, _panel, void 0);
    __privateSet(this, _btn, void 0);
  }
  isOpen() {
    var _a3;
    return "1" === ((_a3 = __privateGet(this, _root)) == null ? void 0 : _a3.getAttribute(SETTINGS_ATTR));
  }
  open() {
    var _a3, _b, _c, _d;
    if (__privateGet(this, _root) === void 0)
      return;
    (_b = (_a3 = __privateGet(this, _host)).onBeforeOpen) == null ? void 0 : _b.call(_a3);
    __privateGet(this, _root).setAttribute(SETTINGS_ATTR, "1");
    (_c = __privateGet(this, _panel)) == null ? void 0 : _c.setAttribute("aria-hidden", "false");
    (_d = __privateGet(this, _btn)) == null ? void 0 : _d.setAttribute("data-active", "1");
    __privateMethod(this, _PresenterSettingsPanel_instances, syncDockLayout_fn).call(this);
  }
  close() {
    var _a3, _b;
    if (__privateGet(this, _root) === void 0)
      return;
    __privateGet(this, _root).setAttribute(SETTINGS_ATTR, "0");
    (_a3 = __privateGet(this, _panel)) == null ? void 0 : _a3.setAttribute("aria-hidden", "true");
    (_b = __privateGet(this, _btn)) == null ? void 0 : _b.setAttribute("data-active", "0");
    __privateMethod(this, _PresenterSettingsPanel_instances, syncDockLayout_fn).call(this);
  }
  toggle() {
    if (this.isOpen())
...
_btn = new WeakMap();
_host = new WeakMap();
_PresenterSettingsPanel_instances = new WeakSet();
syncDockLayout_fn = function() {
  if (__privateGet(this, _root) === void 0)
    return;
  const dock = findDock(__privateGet(this, _root));
  if (dock !== void 0)
    scheduleSyncDockLayout(dock, __privateGet(this, _root));
};
update_fn = function(patch) {
  var _a3, _b;
  const next = patchSettings(patch);
  if (__privateGet(this, _root) !== void 0)
    applyPresenterSettings(__privateGet(this, _root), next);
  __privateMethod(this, _PresenterSettingsPanel_instances, syncUi_fn).call(this);
  (_b = (_a3 = __privateGet(this, _host)).onSettingsChange) == null ? void 0 : _b.call(_a3, next);
};
syncUi_fn = function() {
  var _a3, _b, _c, _d;
  const s = activeSettings;
  const label = (_a3 = __privateGet(this, _panel)) == null ? void 0 : _a3.querySelector("[data-reticle-cycle-label]");
  if (label !== null && label !== void 0) {
    label.textContent = ((_b = OUTPUT_DETAIL_OPTIONS.find((o) => o.value === s.outputDetail)) == null ? void 0 : _b.label) ?? "Standard";
  }
  const dots = (_c = __privateGet(this, _panel)) == null ? void 0 : _c.querySelector("[data-reticle-cycle-dots]");
  ...
  __privateMethod(this, _PresenterSettingsPanel_instances, paintToggle_fn).call(this, "reactComponents", s.reactComponents);
  __privateMethod(this, _PresenterSettingsPanel_instances, paintToggle_fn).call(this, "hideUntilRestart", s.hideUntilRestart);
  __privateMethod(this, _PresenterSettingsPanel_instances, paintToggle_fn).call(this, "showTally", s.showTally);
  __privateMethod(this, _PresenterSettingsPanel_instances, paintToggle_fn).call(this, "autoOpenChat", s.autoOpenChat);
  __privateMethod(this, _PresenterSettingsPanel_instances, paintToggle_fn).call(this, "showTimestamps", s.showTimestamps);
  __privateMethod(this, _PresenterSettingsPanel_instances, paintToggle_fn).call(this, "reduceMotion", s.reduceMotion);
  __privateMethod(this, _PresenterSettingsPanel_instances, paintToggle_fn).call(this, "ambientGlow", s.ambientGlow);
  __privateMethod(this, _PresenterSettingsPanel_instances, paintCheck_fn).call(this, "clearOnCopy", s.clearOnCopy);
  __privateMethod(this, _PresenterSettingsPanel_instances, paintCheck_fn).call(this, "blockPageInteractions", s.blockPageInteractions);
  for (const chip of ((_d = __privateGet(this, _panel)) == null ? void 0 : _d.querySelectorAll("[data-reticle-theme]")) ?? []) {
    if (chip instanceof HTMLElement) {
      chip.setAttribute("data-on", chip.getAttribute("data-reticle-theme") === s.statusThemeId ? "1" : "0");
    }
  }
};
paintToggle_fn = function(key, on) {
  var _a3;
  const el = (_a3 = __privateGet(this, _panel)) == null ? void 0 : _a3.querySelector(`[${SETTING_KEY_ATTR}="${key}"]`);
  if (el instanceof HTMLElement) {
    el.setAttribute("data-on", on ? "1" : "0");
    el.setAttribute("aria-checked", on ? "true" : "false");
  }
};
paintCheck_fn = function(key, on) {
  var _a3;
  const el = (_a3 = __privateGet(this, _panel)) == null ? void 0 : _a3.querySelector(`[data-reticle-check="${key}"]`);
  if (el instanceof HTMLElement) {
    el.setAttribute("data-on", on ? "1" : "0");
    el.setAttribute("aria-checked", on ? "true" : "false");
    el.textContent = on ? "✓" : "";
...
buildThemes_fn = function() {
  var _a3;
  const host = (_a3 = __privateGet(this, _panel)) == null ? void 0 : _a3.querySelector("[data-reticle-settings-themes]");
  ...
  host.replaceChildren(...STATUS_THEMES.map((t) => {
    ...
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      __privateMethod(this, _PresenterSettingsPanel_instances, update_fn).call(this, { statusThemeId: t.id });
    });
    return btn;
  }));
};
...
function paintWorkspace(root) {
  ...
  if (summary.projectId !== void 0) {
    projectRow == null ? void 0 : projectRow.removeAttribute("hidden");
    if (projectEl instanceof HTMLElement)
      projectEl.textContent = summary.projectId;
  } else {
    projectRow == null ? void 0 : projectRow.setAttribute("hidden", "");
    if (projectEl instanceof HTMLElement)
...
function mountWorkspaceSelector(root) {
  ...
  const onCopy = (e) => {
    ...
    if (path === void 0)
      return;
    void ((_a3 = navigator.clipboard) == null ? void 0 : _a3.writeText(path));
    if (copyBtn instanceof HTMLButtonElement) {
  ...
  copyBtn == null ? void 0 : copyBtn.addEventListener("click", onCopy);
  document.addEventListener("pointerdown", onDocPointer);
  document.addEventListener("keydown", onKeyDown);
  return () => {
    btn.removeEventListener("click", onBtnClick);
    copyBtn == null ? void 0 : copyBtn.removeEventListener("click", onCopy);
...
var _refs, _state, _fadeTimer, _root2, _glow, _flowItems, _workspaceTeardown, _listeners, _host2, _ControlPanel_instances, runJson_fn, onCopy_fn, onExport_fn, onPauseToggle_fn, onEnd_fn, onSend_fn, autosize_fn, renderFlows_fn;
var ControlPanel = class {
  constructor(host) {
    __privateAdd(this, _ControlPanel_instances);
    ...
    __privateAdd(this, _state, SessionState.ACTIVE);
    __privateAdd(this, _fadeTimer);
    __privateAdd(this, _root2);
    __privateAdd(this, _glow);
    /** The full replayable-flow list from the last push; re-filtered per page on route change. */
    __privateAdd(this, _flowItems, []);
    __privateAdd(this, _workspaceTeardown);
    /**
  ...
  mount(root, glow) {
    ...
    __privateSet(this, _listeners, new AbortController());
    const { signal } = __privateGet(this, _listeners);
    __privateSet(this, _root2, root);
    __privateSet(this, _glow, glow);
    __privateSet(this, _refs, queryControlRefs(root));
    const pauseBtn = __privateGet(this, _refs).pauseBtn;
    if (pauseBtn !== void 0) {
      paintPauseBtn(pauseBtn, __privateGet(this, _state) === SessionState.PAUSED);
    }
    (_a3 = __privateGet(this, _refs).pauseBtn) == null ? void 0 : _a3.addEventListener("click", () => __privateMethod(this, _ControlPanel_instances, onPauseToggle_fn).call(this), { signal });
    (_b = __privateGet(this, _refs).endBtn) == null ? void 0 : _b.addEventListener("click", () => __privateMethod(this, _ControlPanel_instances, onEnd_fn).call(this), { signal });
    (_c = __privateGet(this, _refs).sendBtn) == null ? void 0 : _c.addEventListener("click", () => __privateMethod(this, _ControlPanel_instances, onSend_fn).call(this), { signal });
    (_d = __privateGet(this, _refs).input) == null ? void 0 : _d.addEventListener("keydown", (e) => {
      if (e instanceof KeyboardEvent && "Enter" === e.key && !e.shiftKey) {
        e.preventDefault();
        __privateMethod(this, _ControlPanel_instances, onSend_fn).call(this);
      }
    }, { signal });
    (_e = __privateGet(this, _refs).input) == null ? void 0 : _e.addEventListener("input", () => __privateMethod(this, _ControlPanel_instances, autosize_fn).call(this), { signal });
    (_f = __privateGet(this, _refs).flows) == null ? void 0 : _f.addEventListener("click", (e) => {
      var _a4;
      const target = e.target;
      if (!(target instanceof HTMLElement))
        return;
      const name = (_a4 = target.closest("[data-reticle-replay]")) == null ? void 0 : _a4.getAttribute("data-reticle-replay");
      if (name !== null && name !== void 0 && name.length > 0) {
        __privateGet(this, _host2).emit(HumanControlKind.REPLAY, name);
      }
    }, { signal });
    (_g = __privateGet(this, _refs).copyBtn) == null ? void 0 : _g.addEventListener("click", () => __privateMethod(this, _ControlPanel_instances, onCopy_fn).call(this), { signal });
    (_h = __privateGet(this, _refs).exportBtn) == null ? void 0 : _h.addEventListener("click", () => __privateMethod(this, _ControlPanel_instances, onExport_fn).call(this), { signal });
    __privateSet(this, _workspaceTeardown, mountWorkspaceSelector(root));
    this.setState(SessionState.ACTIVE);
  }
  /** Clear any pending ended-fade timer (called from Presenter.destroy). */
  teardown() {
    var _a3, _b;
    (_a3 = __privateGet(this, _listeners)) == null ? void 0 : _a3.abort();
    __privateSet(this, _listeners, void 0);
    (_b = __privateGet(this, _workspaceTeardown)) == null ? void 0 : _b.call(this);
    __privateSet(this, _workspaceTeardown, void 0);
    if (__privateGet(this, _fadeTimer) !== void 0)
      nativeClearTimeout(__privateGet(this, _fadeTimer));
    __privateSet(this, _fadeTimer, void 0);
  }
  ...
  setFlows(flows) {
    ...
    __privateSet(this, _flowItems, list.map((f) => {
      if ("string" === typeof f)
    ...
    __privateMethod(this, _ControlPanel_instances, renderFlows_fn).call(this);
  }
  /**
  ...
  setState(state, text, tone) {
    ...
    (_a3 = __privateGet(this, _root2)) == null ? void 0 : _a3.setAttribute(DATA_RETICLE_STATE, state);
    const handoff = tone !== void 0 && tone !== PresenterTone.CALM;
    if (handoff)
      (_b = __privateGet(this, _root2)) == null ? void 0 : _b.setAttribute(DATA_RETICLE_TONE, tone);
    else
      (_c = __privateGet(this, _root2)) == null ? void 0 : _c.removeAttribute(DATA_RETICLE_TONE);
    if (__privateGet(this, _fadeTimer) !== void 0) {
      nativeClearTimeout(__privateGet(this, _fadeTimer));
      __privateSet(this, _fadeTimer, void 0);
    }
    const refs2 = __privateGet(this, _refs);
    ...
    if (refs2.banner !== void 0) {
      ...
      refs2.banner.textContent = handoff && summary.length > 0 ? summary : `${ENDED_BANNER_TEXT}${summary.length > 0 ? ` · ${summary}` : ""}`;
    }
    if (ended) {
      const glow = __privateGet(this, _glow);
      __privateSet(this, _fadeTimer, nativeSetTimeout(() => {
        glow == null ? void 0 : glow.setAttribute(DATA_ON2, GLOW_OFF2);
      }, __privateGet(this, _host2).endedFadeMs));
    }
    (_e = (_d = __privateGet(this, _host2)).onStateChange) == null ? void 0 : _e.call(_d, state);
  }
};
_refs = new WeakMap();
_state = new WeakMap();
_fadeTimer = new WeakMap();
_root2 = new WeakMap();
_glow = new WeakMap();
_flowItems = new WeakMap();
_workspaceTeardown = new WeakMap();
_listeners = new WeakMap();
_host2 = new WeakMap();
_ControlPanel_instances = new WeakSet();
/** Serialize the run state to pretty JSON for Copy/Export. */
runJson_fn = function() {
  return JSON.stringify(__privateGet(this, _host2).runState(), null, 2);
};
/** Copy the run state to the clipboard (with a brief "Copied ✓" flash). */
onCopy_fn = function() {
  var _a3, _b, _c;
  void ((_a3 = navigator.clipboard) == null ? void 0 : _a3.writeText(__privateMethod(this, _ControlPanel_instances, runJson_fn).call(this)));
  if (getPresenterSettings().clearOnCopy) {
    (_c = (_b = __privateGet(this, _host2)).clearRunLog) == null ? void 0 : _c.call(_b);
  }
  const msg = __privateGet(this, _refs).exportMsg;
  if (msg !== void 0) {
...
onExport_fn = function() {
  var _a3, _b;
  const blob = new Blob([__privateMethod(this, _ControlPanel_instances, runJson_fn).call(this)], { type: "application/json" });
  ...
  if (getPresenterSettings().clearOnCopy) {
    (_b = (_a3 = __privateGet(this, _host2)).clearRunLog) == null ? void 0 : _b.call(_a3);
  }
};
onPauseToggle_fn = function() {
  if (__privateGet(this, _state) === SessionState.PAUSED) {
    __privateGet(this, _host2).emit(HumanControlKind.RESUME);
    this.setState(SessionState.ACTIVE);
  } else if (__privateGet(this, _state) === SessionState.ACTIVE) {
    __privateGet(this, _host2).emit(HumanControlKind.PAUSE);
    this.setState(SessionState.PAUSED);
  }
};
onEnd_fn = function() {
  if (__privateGet(this, _state) === SessionState.ENDED)
    return;
  __privateGet(this, _host2).emit(HumanControlKind.END);
...
onSend_fn = function() {
  ...
  if (__privateGet(this, _state) === SessionState.ENDED)
    return;
  const text = (((_a3 = __privateGet(this, _refs).input) == null ? void 0 : _a3.value) ?? "").trim();
  if (0 === text.length)
    return;
  __privateGet(this, _host2).emit(HumanControlKind.MESSAGE, text);
  __privateGet(this, _host2).logHuman(text);
  if (getPresenterSettings().clearOnCopy) {
    (_c = (_b = __privateGet(this, _host2)).clearRunLog) == null ? void 0 : _c.call(_b);
  }
  if (__privateGet(this, _refs).input !== void 0)
    __privateGet(this, _refs).input.value = "";
  __privateMethod(this, _ControlPanel_instances, autosize_fn).call(this);
};
...
autosize_fn = function() {
  const el = __privateGet(this, _refs).input;
  if (el === void 0)
    return;
  el.style.height = "auto";
  el.style.height = `${String(Math.min(el.scrollHeight, MSG_MAX_H))}px`;
};
renderFlows_fn = function() {
  const el = __privateGet(this, _refs).flows;
  if (el === void 0)
    return;
  const doc = el.ownerDocument;
  const testids = new Set(Array.from(doc.querySelectorAll("[data-testid]")).map((n) => n.getAttribute("data-testid")));
  const visible = __privateGet(this, _flowItems).filter((f) => f.start === void 0 || testids.has(f.start));
...
var _emit, _now, _onMark, _shouldBlock, _onCountChange, _root3, _hi, _hiLabel, _selBox, _pop, _pending, _hiTimer, _shakeTimer, _active, _marks, _editing, _pendingKey, _pendingTarget, _markersBtn, _clearBtn, _countEl, _accent, _onClick, _onKeydown, _onMove, _onScroll, _onResize, _mo, _Annotator_instances, toggleMarkers_fn, syncChrome_fn, handleKey_fn, handleClick_fn, scheduleMove_fn, handleMove_fn, hideHighlight_fn, openEdit_fn, currentPoint_fn, openPopover_fn, mountPopover_fn, _pendingResolved, _pendingXY, bindSubmit_fn, submitOpen_fn, shakePopover_fn, sendMark_fn, report_fn, dropPending_fn, dropPin_fn, paintSelection_fn, reposition_fn, repositionPopover_fn, closePopover_fn;
var Annotator = class {
  constructor(deps) {
    __privateAdd(this, _Annotator_instances);
    __privateAdd(this, _emit);
    __privateAdd(this, _now);
    __privateAdd(this, _onMark);
    __privateAdd(this, _shouldBlock);
    __privateAdd(this, _onCountChange);
    __privateAdd(this, _root3);
    __privateAdd(this, _hi);
    __privateAdd(this, _hiLabel);
    __privateAdd(this, _selBox);
    __privateAdd(this, _pop);
    __privateAdd(this, _pending);
    __privateAdd(this, _hiTimer);
    __privateAdd(this, _shakeTimer);
    __privateAdd(this, _active, false);
    __privateAdd(this, _marks, []);
    __privateAdd(this, _editing);
    __privateAdd(this, _pendingKey);
    __privateAdd(this, _pendingTarget);
    __privateAdd(this, _markersBtn);
    __privateAdd(this, _clearBtn);
    __privateAdd(this, _countEl);
    __privateAdd(this, _accent);
    __privateAdd(this, _onClick);
    __privateAdd(this, _onKeydown);
    __privateAdd(this, _onMove);
    __privateAdd(this, _onScroll);
    __privateAdd(this, _onResize);
    __privateAdd(this, _mo);
    __privateAdd(this, _pendingResolved);
    __privateAdd(this, _pendingXY, { x: 0, y: 0 });
    __privateSet(this, _emit, deps.emit);
    __privateSet(this, _now, deps.now);
    __privateSet(this, _onMark, deps.onMark);
    __privateSet(this, _shouldBlock, deps.shouldBlock ?? (() => true));
    __privateSet(this, _onCountChange, deps.onCountChange);
  }
  get active() {
    return __privateGet(this, _active);
  }
  get markCount() {
    return __privateGet(this, _marks).length;
  }
  mount() {
    if (__privateGet(this, _root3) !== void 0 || "undefined" === typeof document)
      return;
    ...
    __privateSet(this, _hi, root.querySelector(sel2("hi")) ?? void 0);
    __privateSet(this, _hiLabel, root.querySelector(sel2("hilabel")) ?? void 0);
    __privateSet(this, _selBox, root.querySelector(sel2("sel")) ?? void 0);
    __privateSet(this, _onClick, (ev) => __privateMethod(this, _Annotator_instances, handleClick_fn).call(this, ev));
    document.addEventListener("click", __privateGet(this, _onClick), { capture: true });
    __privateSet(this, _onMove, (ev) => __privateMethod(this, _Annotator_instances, scheduleMove_fn).call(this, ev));
    document.addEventListener("mousemove", __privateGet(this, _onMove), { passive: true, capture: true });
    __privateSet(this, _onKeydown, (ev) => __privateMethod(this, _Annotator_instances, handleKey_fn).call(this, ev));
    document.addEventListener("keydown", __privateGet(this, _onKeydown));
    __privateSet(this, _onScroll, () => __privateMethod(this, _Annotator_instances, reposition_fn).call(this));
    __privateSet(this, _onResize, () => __privateMethod(this, _Annotator_instances, reposition_fn).call(this));
    window.addEventListener("scroll", __privateGet(this, _onScroll), true);
    window.addEventListener("resize", __privateGet(this, _onResize));
    __privateSet(this, _mo, new MutationObserver(() => this.syncAnchors()));
    __privateGet(this, _mo).observe(document.documentElement, { childList: true, subtree: true });
    if (void 0 !== __privateGet(this, _accent))
      this.setAccent(__privateGet(this, _accent));
  }
  /** Drive marker/highlight chrome from the HUD accent swatch. */
  setAccent(hex) {
    var _a3;
    __privateSet(this, _accent, hex);
    (_a3 = __privateGet(this, _root3)) == null ? void 0 : _a3.style.setProperty("--reticle-mark-accent", hex);
  }
  ...
  attachChrome(chrome) {
    var _a3, _b;
    __privateSet(this, _markersBtn, chrome.markersBtn);
    __privateSet(this, _clearBtn, chrome.clearBtn);
    __privateSet(this, _countEl, chrome.countEl);
    (_a3 = __privateGet(this, _markersBtn)) == null ? void 0 : _a3.addEventListener("click", (e) => {
      e.stopPropagation();
      __privateMethod(this, _Annotator_instances, toggleMarkers_fn).call(this);
    });
    (_b = __privateGet(this, _clearBtn)) == null ? void 0 : _b.addEventListener("click", (e) => {
      e.stopPropagation();
      this.clearAll();
    });
    __privateMethod(this, _Annotator_instances, syncChrome_fn).call(this);
  }
  destroy() {
    var _a3, _b;
    if (__privateGet(this, _onClick) !== void 0) {
      document.removeEventListener("click", __privateGet(this, _onClick), { capture: true });
      __privateSet(this, _onClick, void 0);
    }
    if (__privateGet(this, _onKeydown) !== void 0) {
      document.removeEventListener("keydown", __privateGet(this, _onKeydown));
      __privateSet(this, _onKeydown, void 0);
    }
    if (__privateGet(this, _onMove) !== void 0) {
      document.removeEventListener("mousemove", __privateGet(this, _onMove), { capture: true });
      __privateSet(this, _onMove, void 0);
    }
    if (__privateGet(this, _onScroll) !== void 0) {
      window.removeEventListener("scroll", __privateGet(this, _onScroll), true);
      __privateSet(this, _onScroll, void 0);
    }
    if (__privateGet(this, _onResize) !== void 0) {
      window.removeEventListener("resize", __privateGet(this, _onResize));
      __privateSet(this, _onResize, void 0);
    }
    (_a3 = __privateGet(this, _mo)) == null ? void 0 : _a3.disconnect();
    __privateSet(this, _mo, void 0);
    if (__privateGet(this, _hiTimer) !== void 0)
      nativeClearTimeout(__privateGet(this, _hiTimer));
    if (__privateGet(this, _shakeTimer) !== void 0)
      nativeClearTimeout(__privateGet(this, _shakeTimer));
    __privateMethod(this, _Annotator_instances, closePopover_fn).call(this);
    document.documentElement.removeAttribute(ACTIVE_ATTR);
    (_b = __privateGet(this, _root3)) == null ? void 0 : _b.remove();
    document.querySelectorAll(`style[${MARK_ATTR2}="style"]`).forEach((s) => s.remove());
    __privateSet(this, _root3, void 0);
    __privateSet(this, _active, false);
    __privateSet(this, _marks, []);
  }
  toggle(on) {
    __privateSet(this, _active, on ?? !__privateGet(this, _active));
    if (__privateGet(this, _active))
      document.documentElement.setAttribute(ACTIVE_ATTR, "1");
    else {
      document.documentElement.removeAttribute(ACTIVE_ATTR);
      __privateMethod(this, _Annotator_instances, hideHighlight_fn).call(this);
      __privateMethod(this, _Annotator_instances, closePopover_fn).call(this);
    }
    __privateMethod(this, _Annotator_instances, syncChrome_fn).call(this);
  }
  clearAll() {
    __privateMethod(this, _Annotator_instances, closePopover_fn).call(this);
    for (const mark of __privateGet(this, _marks))
      mark.pin.remove();
    __privateSet(this, _marks, []);
    __privateMethod(this, _Annotator_instances, syncChrome_fn).call(this);
  }
  syncAnchors() {
    ...
    __privateMethod(this, _Annotator_instances, paintSelection_fn).call(this);
  }
};
_emit = new WeakMap();
_now = new WeakMap();
_onMark = new WeakMap();
_shouldBlock = new WeakMap();
_onCountChange = new WeakMap();
_root3 = new WeakMap();
_hi = new WeakMap();
_hiLabel = new WeakMap();
_selBox = new WeakMap();
_pop = new WeakMap();
_pending = new WeakMap();
_hiTimer = new WeakMap();
_shakeTimer = new WeakMap();
_active = new WeakMap();
_marks = new WeakMap();
_editing = new WeakMap();
_pendingKey = new WeakMap();
_pendingTarget = new WeakMap();
_markersBtn = new WeakMap();
_clearBtn = new WeakMap();
...
_mo = new WeakMap();
_Annotator_instances = new WeakSet();
toggleMarkers_fn = function() {
  var _a3, _b;
  const root = __privateGet(this, _root3);
  if (root === void 0)
    return;
  ...
  (_a3 = __privateGet(this, _markersBtn)) == null ? void 0 : _a3.setAttribute("data-active", "0" === hide ? "0" : "1");
  (_b = __privateGet(this, _markersBtn)) == null ? void 0 : _b.setAttribute("aria-pressed", "1" === hide ? "true" : "false");
};
syncChrome_fn = function() {
  var _a3, _b, _c;
  const n = __privateGet(this, _marks).length;
  (_a3 = __privateGet(this, _onCountChange)) == null ? void 0 : _a3.call(this, n);
  if (__privateGet(this, _countEl) !== void 0) {
    __privateGet(this, _countEl).textContent = String(n);
    __privateGet(this, _countEl).hidden = 0 === n;
  }
  (_b = __privateGet(this, _clearBtn)) == null ? void 0 : _b.toggleAttribute("disabled", 0 === n);
  (_c = __privateGet(this, _markersBtn)) == null ? void 0 : _c.toggleAttribute("disabled", 0 === n);
};
handleKey_fn = function(ev) {
  if (ev.key !== "Escape" || !__privateGet(this, _active))
    return;
  if (__privateGet(this, _pop) !== void 0) {
    ev.preventDefault();
    __privateMethod(this, _Annotator_instances, closePopover_fn).call(this);
    return;
  }
...
handleClick_fn = function(ev) {
  ...
  if (isReticleOverlay(target))
    return;
  if (true === ((_a3 = __privateGet(this, _markersBtn)) == null ? void 0 : _a3.contains(target)))
    return;
  if (true === ((_b = __privateGet(this, _clearBtn)) == null ? void 0 : _b.contains(target)))
    return;
  ...
  if (__privateGet(this, _pop) !== void 0) {
    ev.preventDefault();
    ev.stopPropagation();
    __privateMethod(this, _Annotator_instances, shakePopover_fn).call(this);
    return;
  }
  const el = target instanceof HTMLElement ? target : target.parentElement;
  if (null === el)
    return;
  if (__privateGet(this, _shouldBlock).call(this)) {
    ev.preventDefault();
    ev.stopPropagation();
  } else {
    ev.preventDefault();
  ...
  const existing = __privateGet(this, _marks).find((m) => m.anchor === key && m.route === currentRoute());
  if (existing !== void 0) {
    __privateMethod(this, _Annotator_instances, openEdit_fn).call(this, existing);
    return;
  }
  __privateMethod(this, _Annotator_instances, openPopover_fn).call(this, el, ev.clientX, ev.clientY);
};
scheduleMove_fn = function(ev) {
  if (__privateGet(this, _hiTimer) !== void 0)
    nativeClearTimeout(__privateGet(this, _hiTimer));
  __privateSet(this, _hiTimer, nativeSetTimeout(() => {
    __privateSet(this, _hiTimer, void 0);
    __privateMethod(this, _Annotator_instances, handleMove_fn).call(this, ev);
  }, HIGHLIGHT_REST_MS));
};
handleMove_fn = function(ev) {
  if (__privateGet(this, _hi) === void 0)
    return;
  const raw = ev.target;
  const target = raw instanceof Element && raw.hasAttribute(BLOCKER_ATTR_NAME) ? pageElementAt(ev.clientX, ev.clientY) : raw;
  const skip = !__privateGet(this, _active) || __privateGet(this, _pop) !== void 0 || !(target instanceof Element) || isReticleOverlay(target) || target.closest(sel2("pin")) !== null;
  if (skip) {
    __privateGet(this, _hi).setAttribute("data-on", "0");
    return;
  }
  ...
  __privateGet(this, _hi).style.left = `${String(rect.left)}px`;
  __privateGet(this, _hi).style.top = `${String(rect.top)}px`;
  __privateGet(this, _hi).style.width = `${String(rect.width)}px`;
  __privateGet(this, _hi).style.height = `${String(rect.height)}px`;
  __privateGet(this, _hi).setAttribute("data-on", "1");
  if (__privateGet(this, _hiLabel) !== void 0)
    __privateGet(this, _hiLabel).textContent = describeEl(target);
};
hideHighlight_fn = function() {
  var _a3;
  if (__privateGet(this, _hiTimer) !== void 0) {
    nativeClearTimeout(__privateGet(this, _hiTimer));
    __privateSet(this, _hiTimer, void 0);
  }
  (_a3 = __privateGet(this, _hi)) == null ? void 0 : _a3.setAttribute("data-on", "0");
};
openEdit_fn = function(mark) {
  __privateMethod(this, _Annotator_instances, closePopover_fn).call(this);
  __privateSet(this, _editing, mark);
  __privateSet(this, _pendingKey, mark.anchor);
  __privateSet(this, _pendingTarget, mark.target);
  __privateMethod(this, _Annotator_instances, hideHighlight_fn).call(this);
  const at = __privateMethod(this, _Annotator_instances, currentPoint_fn).call(this, mark);
  __privateMethod(this, _Annotator_instances, mountPopover_fn).call(this, mark.label, at.x, at.y, mark.note);
  __privateMethod(this, _Annotator_instances, dropPending_fn).call(this, at.x, at.y);
  __privateMethod(this, _Annotator_instances, paintSelection_fn).call(this);
};
/**
...
openPopover_fn = function(el, x, y) {
  const resolved = resolveMarkAnchor(el);
  const key = resolved.anchor;
  if (__privateGet(this, _pendingKey) === key && __privateGet(this, _pop) !== void 0) {
    __privateMethod(this, _Annotator_instances, shakePopover_fn).call(this);
    return;
  }
  __privateMethod(this, _Annotator_instances, closePopover_fn).call(this);
  __privateMethod(this, _Annotator_instances, hideHighlight_fn).call(this);
  __privateSet(this, _editing, void 0);
  __privateSet(this, _pendingKey, key);
  __privateSet(this, _pendingTarget, el);
  const where = resolved.source !== void 0 ? `${resolved.label} · ${resolved.source.file}:${String(resolved.source.line)}` : resolved.label;
  __privateMethod(this, _Annotator_instances, mountPopover_fn).call(this, where, x, y, "");
  __privateMethod(this, _Annotator_instances, dropPending_fn).call(this, x, y);
  __privateMethod(this, _Annotator_instances, paintSelection_fn).call(this);
  __privateMethod(this, _Annotator_instances, bindSubmit_fn).call(this, resolved, x, y);
};
mountPopover_fn = function(where, x, y, initial) {
  var _a3, _b;
  ...
  (_a3 = __privateGet(this, _root3)) == null ? void 0 : _a3.appendChild(pop);
  __privateSet(this, _pop, pop);
  ...
  if (textarea !== null) {
    ...
    textarea.addEventListener("keydown", (e) => {
      ...
      if ("Escape" === e.key) {
        ...
        e.preventDefault();
        __privateMethod(this, _Annotator_instances, submitOpen_fn).call(this);
      }
    });
    nativeSetTimeout(() => textarea.focus(), 50);
  }
  (_b = pop.querySelector("button[data-cancel]")) == null ? void 0 : _b.addEventListener("click", () => __privateMethod(this, _Annotator_instances, closePopover_fn).call(this));
  send == null ? void 0 : send.addEventListener("click", () => __privateMethod(this, _Annotator_instances, submitOpen_fn).call(this));
  __privateSet(this, _pendingResolved, void 0);
  __privateSet(this, _pendingXY, { x, y });
};
_pendingResolved = new WeakMap();
_pendingXY = new WeakMap();
bindSubmit_fn = function(resolved, x, y) {
  __privateSet(this, _pendingResolved, resolved);
  __privateSet(this, _pendingXY, { x, y });
};
submitOpen_fn = function() {
  var _a3, _b;
  const note = ((_b = (_a3 = __privateGet(this, _pop)) == null ? void 0 : _a3.querySelector("textarea")) == null ? void 0 : _b.value.trim()) ?? "";
  if (0 === note.length)
    return;
  const editing = __privateGet(this, _editing);
  if (editing !== void 0) {
    editing.note = note;
    const tipn = editing.pin.querySelector(sel2("tipn"));
    if (tipn !== null)
      tipn.textContent = note;
    __privateGet(this, _emit).call(this, EventType.HUMAN_MARK, {
      note,
      anchor: editing.anchor,
      label: editing.label,
      route: editing.route
    });
    __privateMethod(this, _Annotator_instances, report_fn).call(this, note, editing.anchor, editing.label, __privateGet(this, _marks).indexOf(editing) + 1, editing.source);
    __privateMethod(this, _Annotator_instances, closePopover_fn).call(this);
    return;
  }
  const resolved = __privateGet(this, _pendingResolved);
  if (resolved === void 0)
    return;
  __privateMethod(this, _Annotator_instances, sendMark_fn).call(this, note, resolved, __privateGet(this, _pendingXY).x, __privateGet(this, _pendingXY).y);
  __privateMethod(this, _Annotator_instances, closePopover_fn).call(this);
};
shakePopover_fn = function() {
  const pop = __privateGet(this, _pop);
  if (pop === void 0)
    return;
  ...
  if (__privateGet(this, _shakeTimer) !== void 0)
    nativeClearTimeout(__privateGet(this, _shakeTimer));
  __privateSet(this, _shakeTimer, nativeSetTimeout(() => {
    pop.classList.remove("reticle-mark-shake");
    __privateSet(this, _shakeTimer, void 0);
  }, SHAKE_MS));
};
sendMark_fn = function(note, resolved, x, y) {
  const data = {
    note,
  ...
  __privateGet(this, _emit).call(this, EventType.HUMAN_MARK, data);
  __privateGet(this, _now).call(this);
  __privateMethod(this, _Annotator_instances, dropPin_fn).call(this, resolved, note, x, y);
  __privateMethod(this, _Annotator_instances, report_fn).call(this, note, resolved.anchor, resolved.label, __privateGet(this, _marks).length, sourceLabel(resolved));
};
/** Hand the HUD everything it needs to name this mark in one row. */
report_fn = function(note, anchor, label, index, source) {
  var _a3;
  const mark = { note, anchor, label, index };
  if (source !== void 0)
    mark.source = source;
  (_a3 = __privateGet(this, _onMark)) == null ? void 0 : _a3.call(this, mark);
};
dropPending_fn = function(x, y) {
  var _a3, _b;
  (_a3 = __privateGet(this, _pending)) == null ? void 0 : _a3.remove();
  ...
  (_b = __privateGet(this, _root3)) == null ? void 0 : _b.appendChild(pending);
  __privateSet(this, _pending, pending);
};
dropPin_fn = function(resolved, note, x, y) {
  if (__privateGet(this, _root3) === void 0)
    return;
  const pin = document.createElement("button");
  pin.type = "button";
  pin.setAttribute(MARK_ATTR2, "pin");
  pin.setAttribute("aria-label", resolved.label);
  const n = __privateGet(this, _marks).length + 1;
  ...
  if (tn !== null)
    tn.textContent = note;
  const isFixed = isElementFixed(__privateGet(this, _pendingTarget));
  ...
  __privateGet(this, _root3).appendChild(pin);
  const mark = {
    id: `${resolved.anchor}:${String(__privateGet(this, _now).call(this))}:${String(n)}`,
    note,
    label: resolved.label,
    anchor: resolved.anchor,
    route: currentRoute(),
    ...
    isFixed,
    pin,
    target: __privateGet(this, _pendingTarget),
  ...
  pin.addEventListener("click", (e) => {
    ...
    if (!__privateGet(this, _active))
      return;
    __privateMethod(this, _Annotator_instances, openEdit_fn).call(this, mark);
  });
  __privateGet(this, _marks).push(mark);
  __privateMethod(this, _Annotator_instances, syncChrome_fn).call(this);
};
paintSelection_fn = function() {
  const box = __privateGet(this, _selBox);
  if (box === void 0)
    return;
  const el = __privateGet(this, _pendingTarget);
  if (el === void 0 || !document.contains(el) || __privateGet(this, _pop) === void 0) {
    box.hidden = true;
    return;
  }
...
reposition_fn = function() {
  ...
  __privateMethod(this, _Annotator_instances, repositionPopover_fn).call(this);
  __privateMethod(this, _Annotator_instances, paintSelection_fn).call(this);
};
/**
 ...
 */
repositionPopover_fn = function() {
  const pop = __privateGet(this, _pop);
  const mark = __privateGet(this, _editing);
...
closePopover_fn = function() {
  var _a3, _b;
  (_a3 = __privateGet(this, _pop)) == null ? void 0 : _a3.remove();
  __privateSet(this, _pop, void 0);
  (_b = __privateGet(this, _pending)) == null ? void 0 : _b.remove();
  __privateSet(this, _pending, void 0);
  __privateSet(this, _pendingKey, void 0);
  __privateSet(this, _pendingTarget, void 0);
  __privateSet(this, _pendingResolved, void 0);
  __privateSet(this, _editing, void 0);
  if (__privateGet(this, _selBox) !== void 0)
    __privateGet(this, _selBox).hidden = true;
};
...
function rememberSessionLabel(explicit, store, generate) {
  const read = () => {
    try {
      const value = store == null ? void 0 : store.getItem(STORAGE_KEY);
  ...
  const write = (value) => {
    try {
      store == null ? void 0 : store.setItem(STORAGE_KEY, value);
    } catch {
    }
  };
...
function isHotLike(value) {
  var _a3;
  return "function" === typeof ((_a3 = fields(value)) == null ? void 0 : _a3["on"]);
}
function updatedPaths(payload) {
  var _a3, _b;
  const updates = (_a3 = fields(payload)) == null ? void 0 : _a3["updates"];
  if (!Array.isArray(updates))
    return [];
  const paths = [];
  for (const update of updates) {
    const path = (_b = fields(update)) == null ? void 0 : _b["path"];
    if ("string" === typeof path && path.length > 0 && !paths.includes(path))
      paths.push(path);
  }
  return paths;
}
var _epoch, _files, _refs2;
var EditEpoch = class {
  ...
  constructor(registry) {
    __privateAdd(this, _epoch, NO_EDITS_OBSERVED);
    __privateAdd(this, _files, []);
    __privateAdd(this, _refs2);
    __privateSet(this, _refs2, registry);
  }
  /**
  ...
  applied(files) {
    __privateSet(this, _epoch, __privateGet(this, _epoch) + 1);
    __privateSet(this, _files, files.slice(0, MAX_NAMED_FILES));
    __privateGet(this, _refs2).markEdited();
  }
  /**
  ...
  staleRefMessage(ref) {
    ...
    if (!__privateGet(this, _refs2).mintedBeforeLastEdit(ref))
      return generic;
    const named = 0 === __privateGet(this, _files).length ? "the page hot-updated" : `${__privateGet(this, _files).join(", ")} hot-updated`;
...
function installStorage(emit) {
  ...
  const origSet = (_a3 = Object.getOwnPropertyDescriptor(proto, "setItem")) == null ? void 0 : _a3.value;
  const origRemove = (_b = Object.getOwnPropertyDescriptor(proto, "removeItem")) == null ? void 0 : _b.value;
  const origClear = (_c = Object.getOwnPropertyDescriptor(proto, "clear")) == null ? void 0 : _c.value;
...
function tauriCapture() {
  ...
  if (typeof (internals == null ? void 0 : internals.invoke) !== "function")
    return void 0;
...
async function captureDesktopWindow(fullPage = false) {
  ...
  const capture = "function" === typeof (channel == null ? void 0 : channel.capture) ? channel.capture : tauriCapture();
  if (capture === void 0) {
...
function capturedMethod(target, key) {
  var _a3;
  return (_a3 = Object.getOwnPropertyDescriptor(target, key)) == null ? void 0 : _a3.value;
}
...
function inspect(ref) {
  ...
  const source = formatSource(sourceFor(el, component == null ? void 0 : component.source));
  ...
  const scroll = {
    ...
    overflowY: (cs == null ? void 0 : cs.overflowY) ?? "visible"
  };
  return {
    ...describe(el),
    ...
    formAction: isButton(el) || isInput(el) ? ((_a3 = el.form) == null ? void 0 : _a3.getAttribute("action")) ?? void 0 : void 0,
    formText: isButton(el) || isInput(el) ? ((_b = el.form) == null ? void 0 : _b.textContent) ?? void 0 : void 0,
...
function listAnimations() {
  ...
  const animations = doc.getAnimations().map((a) => {
    ...
    const timing = effect == null ? void 0 : effect.getTiming();
    const target = (effect == null ? void 0 : effect.target) ?? null;
    return {
      playState: a.playState,
      currentTime: a.currentTime,
      duration: timing == null ? void 0 : timing.duration,
...
var _ws, _churnQueue, _signalQueue, _insertOrder, _closed, _disconnectedSince, _reconnectDelay, _lost, _everConnected, _initialFailures, _warnedUnreachable, _dropped, _gap, _sessionId, _unsubscribeVisible, _deps, _now2, _Transport_instances, onVisible_fn, open_fn, scheduleReopen_fn, reopen_fn, noteInitialFailure_fn, noteOutage_fn, onMessage_fn, sendRaw_fn, sendGapMarker_fn;
var Transport = class {
  constructor(deps) {
    __privateAdd(this, _Transport_instances);
    __privateAdd(this, _ws);
    __privateAdd(this, _churnQueue, []);
    __privateAdd(this, _signalQueue, []);
    __privateAdd(this, _insertOrder, 0);
    __privateAdd(this, _closed, false);
    /** When the current continuous outage began (nativeNow), or undefined while connected. */
    __privateAdd(this, _disconnectedSince);
    /** Current reconnect delay; grows on each failure, resets on a successful open or on focus. */
    __privateAdd(this, _reconnectDelay, RECONNECT_DELAY_MS);
    /** Whether onConnectionLost has already fired for the current outage (fire-once). */
    __privateAdd(this, _lost, false);
    /** True once the socket has opened at least once — gates the "unreachable" first-connect warning. */
    __privateAdd(this, _everConnected, false);
    /** Consecutive failed INITIAL connects (before any success). */
    __privateAdd(this, _initialFailures, 0);
    /** Whether the unreachable warning has fired (fire-once). */
    __privateAdd(this, _warnedUnreachable, false);
    ...
    __privateAdd(this, _unsubscribeVisible);
    __privateAdd(this, _deps);
    __privateAdd(this, _now2);
    __privateSet(this, _deps, deps);
    __privateSet(this, _now2, deps.now ?? nativeNow);
  }
  connect() {
    if ("undefined" === typeof WebSocket)
      return;
    __privateSet(this, _closed, false);
    __privateGet(this, _unsubscribeVisible) ?? __privateSet(this, _unsubscribeVisible, (__privateGet(this, _deps).onVisible ?? subscribeDocumentVisible)(() => __privateMethod(this, _Transport_instances, onVisible_fn).call(this)));
    __privateMethod(this, _Transport_instances, open_fn).call(this);
  }
  sendEvent(event) {
    __privateMethod(this, _Transport_instances, sendRaw_fn).call(this, safeStringify({ kind: MessageKind.EVENT, event }), {
      seq: event.seq,
      t: event.t,
      type: event.type
    });
   ...
   */
  reannounce() {
    if (__privateGet(this, _ws) === void 0 || __privateGet(this, _ws).readyState !== WebSocket.OPEN)
      return;
    __privateGet(this, _ws).send(JSON.stringify(__privateGet(this, _deps).hello()));
  }
  close() {
    var _a3, _b;
    __privateSet(this, _closed, true);
    (_a3 = __privateGet(this, _unsubscribeVisible)) == null ? void 0 : _a3.call(this);
    __privateSet(this, _unsubscribeVisible, void 0);
    (_b = __privateGet(this, _ws)) == null ? void 0 : _b.close();
    __privateSet(this, _ws, void 0);
  }
};
_ws = new WeakMap();
_churnQueue = new WeakMap();
_signalQueue = new WeakMap();
_insertOrder = new WeakMap();
_closed = new WeakMap();
_disconnectedSince = new WeakMap();
_reconnectDelay = new WeakMap();
_lost = new WeakMap();
_everConnected = new WeakMap();
_initialFailures = new WeakMap();
_warnedUnreachable = new WeakMap();
_dropped = new WeakMap();
_gap = new WeakMap();
_sessionId = new WeakMap();
_unsubscribeVisible = new WeakMap();
_deps = new WeakMap();
_now2 = new WeakMap();
 ...
 */
onVisible_fn = function() {
  if (__privateGet(this, _closed))
    return;
  __privateSet(this, _reconnectDelay, RECONNECT_DELAY_MS);
  if (__privateGet(this, _ws) !== void 0)
    return;
  __privateMethod(this, _Transport_instances, open_fn).call(this);
};
open_fn = function() {
  let ws;
  try {
    ws = new WebSocket(__privateGet(this, _deps).url);
  } catch {
    __privateMethod(this, _Transport_instances, noteOutage_fn).call(this);
    __privateMethod(this, _Transport_instances, noteInitialFailure_fn).call(this);
    __privateMethod(this, _Transport_instances, scheduleReopen_fn).call(this);
    return;
  }
  __privateSet(this, _ws, ws);
  ws.onopen = () => {
    var _a3, _b;
    __privateSet(this, _disconnectedSince, void 0);
    __privateSet(this, _lost, false);
    __privateSet(this, _everConnected, true);
    __privateSet(this, _reconnectDelay, RECONNECT_DELAY_MS);
    const greeting = __privateGet(this, _deps).hello();
    __privateGet(this, _sessionId) ?? __privateSet(this, _sessionId, greeting.sessionId);
    ws.send(JSON.stringify(greeting));
    __privateMethod(this, _Transport_instances, sendGapMarker_fn).call(this, ws, greeting.sessionId);
    const replay = [...__privateGet(this, _churnQueue), ...__privateGet(this, _signalQueue)].sort((a, b) => a.order - b.order);
    for (const msg of replay)
      ws.send(msg.text);
    __privateSet(this, _churnQueue, []);
    __privateSet(this, _signalQueue, []);
    (_b = (_a3 = __privateGet(this, _deps)).onConnected) == null ? void 0 : _b.call(_a3);
  };
  ws.onmessage = (event) => {
    const data = event.data;
    void __privateMethod(this, _Transport_instances, onMessage_fn).call(this, "string" === typeof data ? data : String(data));
  };
  ws.onclose = (event) => {
    var _a3, _b;
    __privateSet(this, _ws, void 0);
    if (event.code === WS_POLICY_VIOLATION) {
      ...
      (_b = (_a3 = __privateGet(this, _deps)).onConnectionLost) == null ? void 0 : _b.call(_a3);
      return;
    }
    __privateMethod(this, _Transport_instances, noteOutage_fn).call(this);
    __privateMethod(this, _Transport_instances, noteInitialFailure_fn).call(this);
 ...
 */
scheduleReopen_fn = function() {
  if (__privateGet(this, _closed))
    return;
  const delay = __privateGet(this, _reconnectDelay);
  __privateSet(this, _reconnectDelay, nextReconnectDelay(delay));
  const schedule = __privateGet(this, _deps).schedule ?? nativeSetTimeout;
  schedule(() => __privateMethod(this, _Transport_instances, reopen_fn).call(this), delay);
};
reopen_fn = function() {
  if (__privateGet(this, _closed) || __privateGet(this, _ws) !== void 0)
    return;
  __privateMethod(this, _Transport_instances, open_fn).call(this);
};
/**
 ...
 */
noteInitialFailure_fn = function() {
  var _a3, _b;
  if (__privateGet(this, _everConnected) || __privateGet(this, _warnedUnreachable))
    return;
  __privateSet(this, _initialFailures, __privateGet(this, _initialFailures) + 1);
  if (__privateGet(this, _initialFailures) >= UNREACHABLE_WARN_AFTER) {
    __privateSet(this, _warnedUnreachable, true);
    (_b = (_a3 = __privateGet(this, _deps)).onUnreachable) == null ? void 0 : _b.call(_a3, { url: __privateGet(this, _deps).url, attempts: __privateGet(this, _initialFailures) });
  }
};
/**
 ...
 */
noteOutage_fn = function() {
  var _a3, _b;
  const at = __privateGet(this, _now2).call(this);
  __privateGet(this, _disconnectedSince) ?? __privateSet(this, _disconnectedSince, at);
  if (!__privateGet(this, _lost) && at - __privateGet(this, _disconnectedSince) >= SESSION_LIFECYCLE.BRIDGE_LOST_MS) {
    __privateSet(this, _lost, true);
    (_b = (_a3 = __privateGet(this, _deps)).onConnectionLost) == null ? void 0 : _b.call(_a3);
  }
};
onMessage_fn = async function(text) {
  let parsed;
  try {
  ...
  if (!result2.success) {
    const id = parsed == null ? void 0 : parsed.id;
    if ("string" === typeof id && id.length > 0) {
      __privateMethod(this, _Transport_instances, sendRaw_fn).call(this, safeStringify({
  ...
  if (command.sessionId !== void 0 && command.sessionId !== __privateGet(this, _sessionId)) {
    __privateMethod(this, _Transport_instances, sendRaw_fn).call(this, safeStringify({
      kind: MessageKind.COMMAND_RESULT,
      id: command.id,
      ok: false,
      error: `command addressed to session ${command.sessionId} but this page is ${__privateGet(this, _sessionId) ?? "(none)"} — the page reconnected under a new id, or two sessions share a socket. Nothing ran.`
    }));
    return;
  }
  let outcome;
  try {
    outcome = await __privateGet(this, _deps).handleCommand(command);
  } catch (error) {
    outcome = {
      ok: false,
...
sendRaw_fn = function(text, event) {
  if (__privateGet(this, _ws) !== void 0 && __privateGet(this, _ws).readyState === WebSocket.OPEN) {
    __privateGet(this, _ws).send(text);
    return;
  }
  if (__privateGet(this, _churnQueue).length + __privateGet(this, _signalQueue).length >= MAX_QUEUE) {
    const evicted = __privateGet(this, _churnQueue).length > 0 ? __privateGet(this, _churnQueue).shift() : __privateGet(this, _signalQueue).shift();
    if ((evicted == null ? void 0 : evicted.event) !== void 0) {
      __privateSet(this, _dropped, __privateGet(this, _dropped) + 1);
      const previous = __privateGet(this, _gap);
      if (previous === void 0 || evicted.event.t < previous.t)
        __privateSet(this, _gap, evicted.event);
    }
  }
  const isChurn = event !== void 0 && CHURN_TYPES.has(event.type);
  const order = __privateWrapper(this, _insertOrder)._++;
  if (isChurn) {
    __privateGet(this, _churnQueue).push({ text, event, order });
  } else {
    __privateGet(this, _signalQueue).push({ text, event, order });
  }
};
/**
 ...
 */
sendGapMarker_fn = function(ws, sessionId) {
  const gap = __privateGet(this, _gap);
  if (gap === void 0)
    return;
  const marker = {
    t: gap.t,
    ...
    sessionId,
    data: { dropped: __privateGet(this, _dropped) }
  };
  ws.send(safeStringify({ kind: MessageKind.EVENT, event: marker }));
  __privateSet(this, _gap, void 0);
  __privateSet(this, _dropped, 0);
};
...
function regionKeyOf(target) {
  ...
  return (labelled == null ? void 0 : labelled.getAttribute("data-testid")) ?? refs.refFor(el);
}
var WATCHED_ATTRS = [
  "class",
  "hidden",
    ...
    for (const record3 of records) {
      ...
      if ("characterData" === record3.type) {
        ...
        if (parent !== null && !isReticleOverlay(parent)) {
          ...
          const old = capValue(((_a3 = record3.oldValue) == null ? void 0 : _a3.trim()) ?? null);
...
function installStoreState(emit) {
  ...
  const watch = ([name, getter, subscribe]) => {
    var _a3;
    try {
      (_a3 = active.get(name)) == null ? void 0 : _a3();
    } catch {
    }
...
function watchStreamedBody(emit, res, id, url, contentType, contentLength) {
  ...
  if (!isStreamingBody(contentType, contentLength))
    return;
  if (typeof ((_a3 = res.body) == null ? void 0 : _a3.getReader) !== "function")
    return;
  let clone;
  try {
    clone = res.clone();
...
function projectRequestBody(body, captureBodies) {
  ...
  if ("string" === typeof body) {
    ...
    const shape = ((_a3 = body.constructor) == null ? void 0 : _a3.name) ?? typeof body;
...
function methodOf(input, init) {
  if ((init == null ? void 0 : init.method) !== void 0)
...
function installNetwork(emit, opts = {}) {
  ...
  window.fetch = async (input, init) => {
    ...
    if (true === ((_a4 = opts.ignore) == null ? void 0 : _a4.call(opts, rawUrl)))
    ...
    try {
      ...
      const emitRequest = (responseBodyFields) => {
        emit(EventType.NET_REQUEST, {
          ...
          ...projectRequestBody(init == null ? void 0 : init.body, captureBodies),
          ...responseBodyFields,
          ...
          ...(reinterpret == null ? void 0 : reinterpret(url, (name) => res.headers.get(name))) ?? {}
        });
      };
  ...
  if (captureBodies && "function" === typeof origWebSocket) {
    window.WebSocket = (_a3 = class extends origWebSocket {
      constructor(u, protocols) {
        super(u, protocols);
        /** Reticle's own bridge socket is never observed — see isBridgeSocket. */
        __privateAdd(this, _isBridge);
        __privateSet(this, _isBridge, isBridgeSocket(String(u)));
        if (__privateGet(this, _isBridge))
          return;
        const url = redactUrl(String(u));
        emit(EventType.NET_STREAM, {
    ...
    }, _isBridge = new WeakMap(), _a3);
    patchedWebSocket = window.WebSocket;
  }
  const navProto = typeof navigator !== "undefined" ? Object.getPrototypeOf(navigator) : null;
  const origBeacon = null === navProto ? void 0 : (_b = Object.getOwnPropertyDescriptor(navProto, "sendBeacon")) == null ? void 0 : _b.value;
  let patchedBeacon;
  ...
  return () => {
    subresourceObserver == null ? void 0 : subresourceObserver.disconnect();
...
function ipcNetOverrides(url, header) {
  ...
  const command = match == null ? void 0 : match[1];
  if (command === void 0)
    return void 0;
...
function isReticleOwnIpc(url) {
  var _a3;
  return ((_a3 = TAURI_IPC_URL.exec(url)) == null ? void 0 : _a3[1]) === RETICLE_TAURI_CAPTURE_COMMAND;
}
...
function installIpc(emit, options = {}) {
  const channel = window[RETICLE_IPC_GLOBAL];
  if (typeof (channel == null ? void 0 : channel.subscribe) !== "function") {
  ...
  return () => {
    var _a3;
    (_a3 = channel.unsubscribe) == null ? void 0 : _a3.call(channel, token);
  };
}
...
function detectBrand() {
  ...
  try {
    const nav = navigator;
    const brands = (_a3 = nav.userAgentData) == null ? void 0 : _a3.brands;
    if (Array.isArray(brands)) {
      let chrome;
      for (const entry of brands) {
        const name = "string" === typeof (entry == null ? void 0 : entry.brand) ? entry.brand.toLowerCase() : "";
...
function reportBodyHtml(scope) {
  const c = scope.counts;
  if (0 === c.calls)
    return `<p class="reticle-report-empty">${REPORT_TEXT.EMPTY}</p>`;
  const streak = scope.records.streakDays > 0 ? `<span class="reticle-report-streak" title="Consecutive days with at least one verdict">🔥 ${String(scope.records.streakDays)} <span class="reticle-report-streak-label">${REPORT_TEXT.STREAK}</span></span>` : "";
...
var _panel2, _body, _root4, _snapshot, _global, _host3, _PresenterReport_instances, scope_fn, paint_fn, openShare_fn, copy_fn;
var PresenterReport = class {
  constructor(host = {}) {
    __privateAdd(this, _PresenterReport_instances);
    __privateAdd(this, _panel2);
    __privateAdd(this, _body);
    __privateAdd(this, _root4);
    __privateAdd(this, _snapshot);
    __privateAdd(this, _global, false);
    __privateAdd(this, _host3);
    __privateSet(this, _host3, host);
  }
  mount(root) {
    var _a3, _b, _c, _d, _e;
    __privateSet(this, _root4, root);
    __privateSet(this, _panel2, root.querySelector(`[${REPORT_PANEL_ATTR}]`) ?? void 0);
    __privateSet(this, _body, root.querySelector("[data-reticle-report-body]") ?? void 0);
    (_a3 = root.querySelector(`[${REPORT_CLOSE_ATTR}]`)) == null ? void 0 : _a3.addEventListener("click", (e) => {
      e.stopPropagation();
      this.close();
    });
    const scopeBtn = root.querySelector(`[${SCOPE_ATTR}]`);
    scopeBtn == null ? void 0 : scopeBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      __privateSet(this, _global, !__privateGet(this, _global));
      scopeBtn.setAttribute("aria-pressed", __privateGet(this, _global) ? "true" : "false");
      scopeBtn.textContent = __privateGet(this, _global) ? REPORT_TEXT.GLOBAL : REPORT_TEXT.PROJECT;
      __privateMethod(this, _PresenterReport_instances, paint_fn).call(this);
    });
    (_b = root.querySelector(`[${SHARE_X_ATTR}]`)) == null ? void 0 : _b.addEventListener("click", (e) => {
      e.stopPropagation();
      __privateMethod(this, _PresenterReport_instances, openShare_fn).call(this, buildXShareUrl(this.shareText()));
    });
    (_c = root.querySelector(`[${SHARE_IN_ATTR}]`)) == null ? void 0 : _c.addEventListener("click", (e) => {
      e.stopPropagation();
      void __privateMethod(this, _PresenterReport_instances, copy_fn).call(this, this.shareText());
      __privateMethod(this, _PresenterReport_instances, openShare_fn).call(this, buildLinkedInShareUrl());
    });
    (_d = root.querySelector(`[${SHARE_COPY_ATTR}]`)) == null ? void 0 : _d.addEventListener("click", (e) => {
      e.stopPropagation();
      void __privateMethod(this, _PresenterReport_instances, copy_fn).call(this, this.shareText(), e.currentTarget);
    });
    (_e = root.querySelector(`[${REFER_ATTR}]`)) == null ? void 0 : _e.addEventListener("click", (e) => {
      e.stopPropagation();
      void __privateMethod(this, _PresenterReport_instances, copy_fn).call(this, buildReferralText(), e.currentTarget);
    });
  }
  ...
  snapshot() {
    return __privateGet(this, _snapshot);
  }
  isOpen() {
    var _a3;
    return "1" === ((_a3 = __privateGet(this, _root4)) == null ? void 0 : _a3.getAttribute(REPORT_ATTR));
  }
  open() {
    var _a3, _b, _c;
    if (__privateGet(this, _root4) === void 0)
      return;
    (_b = (_a3 = __privateGet(this, _host3)).onBeforeOpen) == null ? void 0 : _b.call(_a3);
    __privateMethod(this, _PresenterReport_instances, paint_fn).call(this);
    __privateGet(this, _root4).setAttribute(REPORT_ATTR, "1");
    (_c = __privateGet(this, _panel2)) == null ? void 0 : _c.setAttribute("aria-hidden", "false");
  }
  close() {
    var _a3, _b;
    (_a3 = __privateGet(this, _root4)) == null ? void 0 : _a3.setAttribute(REPORT_ATTR, "0");
    (_b = __privateGet(this, _panel2)) == null ? void 0 : _b.setAttribute("aria-hidden", "true");
  }
  toggle() {
    if (this.isOpen())
      this.close();
  ...
  contains(node) {
    var _a3;
    return true === ((_a3 = __privateGet(this, _panel2)) == null ? void 0 : _a3.contains(node));
  }
  /** The post text for whichever scope is showing. */
  shareText() {
    const snap = __privateGet(this, _snapshot);
    if (snap === void 0)
      return buildReferralText();
    return __privateGet(this, _global) ? buildShareText(snap.global) : buildShareText(snap.project, snap.projectName);
  }
};
_panel2 = new WeakMap();
_body = new WeakMap();
...
_global = new WeakMap();
_host3 = new WeakMap();
_PresenterReport_instances = new WeakSet();
scope_fn = function() {
  const snap = __privateGet(this, _snapshot);
  if (snap === void 0)
    return void 0;
  return __privateGet(this, _global) ? snap.global : snap.project;
};
paint_fn = function() {
  const scope = __privateMethod(this, _PresenterReport_instances, scope_fn).call(this);
  if (__privateGet(this, _body) === void 0)
    return;
  __privateGet(this, _body).innerHTML = scope === void 0 ? `<p class="reticle-report-empty">${REPORT_TEXT.EMPTY}</p>` : reportBodyHtml(scope);
};
openShare_fn = function(url) {
  window.open(url, "_blank", "noopener,noreferrer");
};
copy_fn = async function(text, button) {
  var _a3;
  try {
    await ((_a3 = navigator.clipboard) == null ? void 0 : _a3.writeText(text));
  } catch {
  }
...
var EXIT_LABEL = "Exit";
var _root5, _dock, _fab, _chatPanel, _chatToggle, _collapseBtn, _settings, _dragTeardown, _layoutTeardown, _transitionLock, _suppressFabClick, _annotateBtn, _annotateOn, _toggleSync, _listeners2, _report, _callbacks, _HudShell_instances, syncToolbarToggles_fn, lockTransition_fn, _onDocPointerDown, _onKeyDown;
var HudShell = class {
  constructor(callbacks = {}) {
    __privateAdd(this, _HudShell_instances);
    __privateAdd(this, _root5);
    __privateAdd(this, _dock);
    __privateAdd(this, _fab);
    __privateAdd(this, _chatPanel);
    __privateAdd(this, _chatToggle);
    __privateAdd(this, _collapseBtn);
    __privateAdd(this, _settings);
    __privateAdd(this, _dragTeardown);
    __privateAdd(this, _layoutTeardown);
    __privateAdd(this, _transitionLock, false);
    __privateAdd(this, _suppressFabClick, false);
    __privateAdd(this, _annotateBtn);
    /**
     ...
     */
    __privateAdd(this, _annotateOn, false);
    __privateAdd(this, _toggleSync);
    /**
    ...
    __privateAdd(this, _report, new PresenterReport({
      onBeforeOpen: () => {
        __privateGet(this, _settings).close();
        this.closeChat();
      }
    }));
     ...
     */
    __privateAdd(this, _onDocPointerDown, (e) => {
      if (__privateGet(this, _root5) === void 0 || __privateGet(this, _dock) === void 0)
        return;
      if (!__privateGet(this, _settings).isOpen())
        return;
      const target = e.target;
      if (!(target instanceof Node))
        return;
      if (__privateGet(this, _dock).contains(target))
        return;
      if (__privateGet(this, _settings).contains(target))
        return;
      __privateGet(this, _settings).close();
    });
    __privateAdd(this, _onKeyDown, (e) => {
      if ("Escape" !== e.key || __privateGet(this, _root5) === void 0)
        return;
      const target = e.target;
    ...
    __privateSet(this, _settings, new PresenterSettingsPanel({
      ...callbacks.settings,
      onBeforeOpen: () => {
        var _a3, _b;
        this.closeChat();
        __privateGet(this, _report).close();
        (_b = (_a3 = callbacks.settings) == null ? void 0 : _a3.onBeforeOpen) == null ? void 0 : _b.call(_a3);
      }
    }));
  }
  ...
  get report() {
    return __privateGet(this, _report);
  }
  /** Does the user currently want to annotate? */
  isAnnotateOn() {
    return __privateGet(this, _annotateOn);
  }
  /** Set the toggle and reflect it on the button, without firing the callback. */
  setAnnotateOn(on) {
    var _a3, _b;
    __privateSet(this, _annotateOn, on);
    (_a3 = __privateGet(this, _annotateBtn)) == null ? void 0 : _a3.setAttribute("aria-pressed", on ? "true" : "false");
    (_b = __privateGet(this, _annotateBtn)) == null ? void 0 : _b.setAttribute("data-active", on ? "1" : "0");
  }
  mount(root) {
    var _a3, _b, _c, _d, _e;
    __privateSet(this, _listeners2, new AbortController());
    const { signal } = __privateGet(this, _listeners2);
    __privateSet(this, _root5, root);
    __privateSet(this, _dock, root.querySelector(`[${DOCK_ATTR}]`) ?? void 0);
    const fabEl = root.querySelector(`[${FAB_ATTR}]`);
    __privateSet(this, _fab, fabEl instanceof HTMLButtonElement ? fabEl : void 0);
    const chatPanelEl = root.querySelector(`[${CHAT_PANEL_ATTR}]`);
    __privateSet(this, _chatPanel, chatPanelEl instanceof HTMLElement ? chatPanelEl : void 0);
    const chatToggleEl = root.querySelector(`[${CHAT_TOGGLE_ATTR}]`);
    __privateSet(this, _chatToggle, chatToggleEl instanceof HTMLElement ? chatToggleEl : void 0);
    const annotateEl = root.querySelector(`[${ANNOTATE_BTN_ATTR}]`);
    __privateSet(this, _annotateBtn, annotateEl instanceof HTMLButtonElement ? annotateEl : void 0);
    (_a3 = __privateGet(this, _annotateBtn)) == null ? void 0 : _a3.addEventListener("click", (e) => {
      var _a4, _b2;
      e.preventDefault();
      e.stopPropagation();
      this.setAnnotateOn(!__privateGet(this, _annotateOn));
      (_b2 = (_a4 = __privateGet(this, _callbacks)).onAnnotateToggle) == null ? void 0 : _b2.call(_a4, __privateGet(this, _annotateOn));
    }, { signal });
    const pillEl = root.querySelector(`[${CHAT_PILL_ATTR}]`);
    pillEl == null ? void 0 : pillEl.addEventListener("click", (e) => {
      e.preventDefault();
      e.stopPropagation();
      this.showChat();
    }, { signal });
    const chatMinEl = root.querySelector(`[${CHAT_MIN_ATTR}]`);
    chatMinEl == null ? void 0 : chatMinEl.addEventListener("click", (e) => {
      e.preventDefault();
      e.stopPropagation();
      this.closeChat();
    }, { signal });
    const collapseEl = root.querySelector("[data-reticle-min-btn]");
    __privateSet(this, _collapseBtn, collapseEl instanceof HTMLButtonElement ? collapseEl : void 0);
    root.setAttribute(MIN_ATTR, "1");
    root.setAttribute(SETTINGS_ATTR, "0");
    root.removeAttribute(CHAT_ATTR);
    __privateGet(this, _settings).mount(root);
    __privateGet(this, _report).mount(root);
    __privateSet(this, _toggleSync, new MutationObserver(() => __privateMethod(this, _HudShell_instances, syncToolbarToggles_fn).call(this)));
    __privateGet(this, _toggleSync).observe(root, {
      attributes: true,
      attributeFilter: [CHAT_ATTR, SETTINGS_ATTR, REPORT_ATTR]
    });
    __privateMethod(this, _HudShell_instances, syncToolbarToggles_fn).call(this);
    (_b = root.querySelector(`[${REPORT_BTN_ATTR}]`)) == null ? void 0 : _b.addEventListener("click", (e) => {
      e.preventDefault();
      e.stopPropagation();
      __privateGet(this, _report).toggle();
    }, { signal });
    (_c = __privateGet(this, _fab)) == null ? void 0 : _c.addEventListener("click", (e) => {
      e.stopPropagation();
      if (__privateGet(this, _suppressFabClick)) {
        __privateSet(this, _suppressFabClick, false);
        return;
      }
      this.expand();
    }, { signal });
    (_d = __privateGet(this, _collapseBtn)) == null ? void 0 : _d.addEventListener("click", (e) => {
      e.stopPropagation();
      this.collapse();
    }, { signal });
    (_e = __privateGet(this, _chatToggle)) == null ? void 0 : _e.addEventListener("click", (e) => {
      e.stopPropagation();
      this.toggleChat();
    }, { signal });
    const toolbarDrag = root.querySelector(`.${DRAG_HANDLE_CLASS}`);
    const dragHandles = [__privateGet(this, _fab), toolbarDrag].filter((el) => el instanceof HTMLElement);
    if (__privateGet(this, _dock) !== void 0 && dragHandles.length > 0) {
      __privateSet(this, _dragTeardown, installHudDragHandles(__privateGet(this, _dock), dragHandles, {
        onDragMove: () => {
          __privateSet(this, _suppressFabClick, true);
        },
        onDragEnd: (moved) => {
      ...
      __privateSet(this, _layoutTeardown, installHudPositionGuards(__privateGet(this, _dock), root));
    }
    document.addEventListener("pointerdown", __privateGet(this, _onDocPointerDown), { signal });
    document.addEventListener("keydown", __privateGet(this, _onKeyDown), { signal });
  }
  teardown() {
    var _a3, _b, _c, _d;
    (_a3 = __privateGet(this, _listeners2)) == null ? void 0 : _a3.abort();
    __privateSet(this, _listeners2, void 0);
    (_b = __privateGet(this, _toggleSync)) == null ? void 0 : _b.disconnect();
    __privateSet(this, _toggleSync, void 0);
    (_c = __privateGet(this, _dragTeardown)) == null ? void 0 : _c.call(this);
    __privateSet(this, _dragTeardown, void 0);
    (_d = __privateGet(this, _layoutTeardown)) == null ? void 0 : _d.call(this);
    __privateSet(this, _layoutTeardown, void 0);
    __privateGet(this, _settings).teardown();
    __privateSet(this, _root5, void 0);
    __privateSet(this, _dock, void 0);
    __privateSet(this, _fab, void 0);
    __privateSet(this, _chatPanel, void 0);
    __privateSet(this, _chatToggle, void 0);
    __privateSet(this, _collapseBtn, void 0);
  }
  isCollapsed() {
    var _a3;
    return "1" === ((_a3 = __privateGet(this, _root5)) == null ? void 0 : _a3.getAttribute(MIN_ATTR));
  }
  isChatOpen() {
    var _a3;
    return "1" === ((_a3 = __privateGet(this, _root5)) == null ? void 0 : _a3.getAttribute(CHAT_ATTR));
  }
  expand() {
    var _a3, _b;
    if (__privateGet(this, _root5) === void 0 || !this.isCollapsed())
      return;
    if (__privateGet(this, _transitionLock))
      return;
    __privateMethod(this, _HudShell_instances, lockTransition_fn).call(this);
    __privateGet(this, _root5).setAttribute(MIN_ATTR, "0");
    if (__privateGet(this, _fab) !== void 0)
      __privateGet(this, _fab).setAttribute("aria-expanded", "true");
    (_b = (_a3 = __privateGet(this, _callbacks)).onExpand) == null ? void 0 : _b.call(_a3);
    this.openChat();
    if (__privateGet(this, _dock) !== void 0)
      scheduleSyncDockLayout(__privateGet(this, _dock), __privateGet(this, _root5));
  }
  collapse() {
    var _a3, _b;
    if (__privateGet(this, _root5) === void 0)
      return;
    __privateSet(this, _transitionLock, false);
    this.closeChat();
    __privateGet(this, _settings).close();
    __privateGet(this, _root5).setAttribute(MIN_ATTR, "1");
    if (__privateGet(this, _fab) !== void 0)
      __privateGet(this, _fab).setAttribute("aria-expanded", "false");
    (_b = (_a3 = __privateGet(this, _callbacks)).onCollapse) == null ? void 0 : _b.call(_a3);
  }
  openChat() {
    var _a3, _b, _c, _d, _e;
    if (__privateGet(this, _root5) === void 0)
    ...
    if (this.isChatOpen())
      return;
    __privateGet(this, _root5).setAttribute(CHAT_ATTR, "1");
    const log = __privateGet(this, _root5).querySelector("[data-reticle-log]");
    if (log !== null)
      settleLogAtLatest(log);
    (_a3 = __privateGet(this, _chatPanel)) == null ? void 0 : _a3.setAttribute("aria-hidden", "false");
    (_b = __privateGet(this, _chatToggle)) == null ? void 0 : _b.setAttribute("data-active", "1");
    (_c = __privateGet(this, _chatToggle)) == null ? void 0 : _c.setAttribute("aria-pressed", "true");
    (_e = (_d = __privateGet(this, _callbacks)).onChatOpen) == null ? void 0 : _e.call(_d);
    if (__privateGet(this, _dock) !== void 0)
      scheduleSyncDockLayout(__privateGet(this, _dock), __privateGet(this, _root5));
    const input = __privateGet(this, _root5).querySelector("[data-reticle-input]");
  ...
  closeChat() {
    var _a3, _b, _c, _d, _e;
    if (__privateGet(this, _root5) === void 0 || !this.isChatOpen())
      return;
    __privateGet(this, _root5).removeAttribute(CHAT_ATTR);
    (_a3 = __privateGet(this, _chatPanel)) == null ? void 0 : _a3.setAttribute("aria-hidden", "true");
    (_b = __privateGet(this, _chatToggle)) == null ? void 0 : _b.setAttribute("data-active", "0");
    (_c = __privateGet(this, _chatToggle)) == null ? void 0 : _c.setAttribute("aria-pressed", "false");
    (_e = (_d = __privateGet(this, _callbacks)).onChatClose) == null ? void 0 : _e.call(_d);
    if (__privateGet(this, _dock) !== void 0)
      scheduleSyncDockLayout(__privateGet(this, _dock), __privateGet(this, _root5));
  }
  toggleChat() {
  ...
  pulseFab(active) {
    var _a3;
    (_a3 = __privateGet(this, _fab)) == null ? void 0 : _a3.setAttribute("data-pulse", active ? "1" : "0");
  }
};
_root5 = new WeakMap();
_dock = new WeakMap();
_fab = new WeakMap();
_chatPanel = new WeakMap();
_chatToggle = new WeakMap();
_collapseBtn = new WeakMap();
_settings = new WeakMap();
_dragTeardown = new WeakMap();
_layoutTeardown = new WeakMap();
_transitionLock = new WeakMap();
_suppressFabClick = new WeakMap();
_annotateBtn = new WeakMap();
_annotateOn = new WeakMap();
_toggleSync = new WeakMap();
_listeners2 = new WeakMap();
_report = new WeakMap();
_callbacks = new WeakMap();
_HudShell_instances = new WeakSet();
/** Light exactly the toolbar buttons whose panels are open. */
syncToolbarToggles_fn = function() {
  const root = __privateGet(this, _root5);
  if (root === void 0)
    return;
  const lit = (btnAttr, on) => {
    const btn = root.querySelector(`[${btnAttr}]`);
    btn == null ? void 0 : btn.setAttribute("data-active", on ? "1" : "0");
    btn == null ? void 0 : btn.setAttribute("aria-pressed", on ? "true" : "false");
  };
...
lockTransition_fn = function() {
  __privateSet(this, _transitionLock, true);
  window.setTimeout(() => {
    __privateSet(this, _transitionLock, false);
  }, TRANSITION_LOCK_MS);
};
_onDocPointerDown = new WeakMap();
_onKeyDown = new WeakMap();
...
var _phase, _lastActivityMs, _idleCheckTimer, _fadeTimer2, _glow2, _cursor, _now3, _idleAfterMs, _glowFadeMs, _borderMode, _setMode, _GlowController_instances, enterBusy_fn, armIdleCheck_fn, checkIdle_fn, beginFade_fn;
var GlowController = class {
  constructor(deps) {
    __privateAdd(this, _GlowController_instances);
    __privateAdd(this, _phase, GlowPhase.IDLE);
    __privateAdd(this, _lastActivityMs, 0);
    __privateAdd(this, _idleCheckTimer);
    __privateAdd(this, _fadeTimer2);
    __privateAdd(this, _glow2);
    __privateAdd(this, _cursor);
    __privateAdd(this, _now3);
    __privateAdd(this, _idleAfterMs);
    __privateAdd(this, _glowFadeMs);
    __privateAdd(this, _borderMode);
    __privateAdd(this, _setMode);
    __privateSet(this, _now3, deps.now);
    __privateSet(this, _idleAfterMs, deps.idleAfterMs);
    __privateSet(this, _glowFadeMs, deps.glowFadeMs);
    __privateSet(this, _borderMode, deps.borderMode);
    __privateSet(this, _setMode, deps.setMode);
  }
  /** Wire the glow + cursor elements after the Presenter mounts the DOM. */
  setElements(glow, cursor) {
    __privateSet(this, _glow2, glow);
  ...
  lastActivityMs() {
    return __privateGet(this, _lastActivityMs);
  }
  /** Set the activity baseline WITHOUT entering busy (sessionStart / revive). */
  resetActivity(ms) {
    __privateSet(this, _lastActivityMs, ms);
  }
  /**
   ...
   */
  markActivity(ms = __privateGet(this, _now3).call(this)) {
    __privateSet(this, _lastActivityMs, ms);
    if (__privateGet(this, _phase) === GlowPhase.IDLE || __privateGet(this, _phase) === GlowPhase.FADING)
      __privateMethod(this, _GlowController_instances, enterBusy_fn).call(this);
    __privateMethod(this, _GlowController_instances, armIdleCheck_fn).call(this);
  }
  ...
  teardown() {
    if (__privateGet(this, _idleCheckTimer) !== void 0)
      nativeClearTimeout(__privateGet(this, _idleCheckTimer));
    if (__privateGet(this, _fadeTimer2) !== void 0)
      nativeClearTimeout(__privateGet(this, _fadeTimer2));
    __privateSet(this, _idleCheckTimer, void 0);
    __privateSet(this, _fadeTimer2, void 0);
  }
};
_phase = new WeakMap();
_lastActivityMs = new WeakMap();
_idleCheckTimer = new WeakMap();
_fadeTimer2 = new WeakMap();
_glow2 = new WeakMap();
_cursor = new WeakMap();
_now3 = new WeakMap();
_idleAfterMs = new WeakMap();
_glowFadeMs = new WeakMap();
_borderMode = new WeakMap();
_setMode = new WeakMap();
_GlowController_instances = new WeakSet();
enterBusy_fn = function() {
  var _a3, _b, _c;
  if (__privateGet(this, _fadeTimer2) !== void 0) {
    nativeClearTimeout(__privateGet(this, _fadeTimer2));
    __privateSet(this, _fadeTimer2, void 0);
  }
  __privateSet(this, _phase, GlowPhase.BUSY);
  if (__privateGet(this, _borderMode) === BorderMode.SESSION) {
    (_a3 = __privateGet(this, _glow2)) == null ? void 0 : _a3.setAttribute(DATA_BUSY, BUSY_ON);
  } else {
    (_b = __privateGet(this, _glow2)) == null ? void 0 : _b.setAttribute(DATA_ON, GLOW_ON);
  }
  (_c = __privateGet(this, _cursor)) == null ? void 0 : _c.setAttribute(DATA_ON, GLOW_ON);
};
armIdleCheck_fn = function() {
  if (__privateGet(this, _idleCheckTimer) !== void 0)
    nativeClearTimeout(__privateGet(this, _idleCheckTimer));
  __privateSet(this, _idleCheckTimer, nativeSetTimeout(() => __privateMethod(this, _GlowController_instances, checkIdle_fn).call(this), __privateGet(this, _idleAfterMs)));
};
checkIdle_fn = function() {
  __privateSet(this, _idleCheckTimer, void 0);
  if (__privateGet(this, _phase) !== GlowPhase.BUSY)
    return;
  const quietFor = __privateGet(this, _now3).call(this) - __privateGet(this, _lastActivityMs);
  if (quietFor < __privateGet(this, _idleAfterMs)) {
    __privateSet(this, _idleCheckTimer, nativeSetTimeout(() => __privateMethod(this, _GlowController_instances, checkIdle_fn).call(this), __privateGet(this, _idleAfterMs) - quietFor));
    return;
  }
  __privateMethod(this, _GlowController_instances, beginFade_fn).call(this);
};
beginFade_fn = function() {
  var _a3, _b, _c;
  __privateSet(this, _phase, GlowPhase.FADING);
  if (__privateGet(this, _borderMode) === BorderMode.SESSION) {
    (_a3 = __privateGet(this, _glow2)) == null ? void 0 : _a3.setAttribute(DATA_BUSY, BUSY_OFF);
  } else {
    (_b = __privateGet(this, _glow2)) == null ? void 0 : _b.setAttribute(DATA_ON, GLOW_OFF);
  }
  (_c = __privateGet(this, _cursor)) == null ? void 0 : _c.setAttribute(DATA_ON, GLOW_OFF);
  __privateGet(this, _setMode).call(this, PresenterMode.IDLE);
  __privateSet(this, _fadeTimer2, nativeSetTimeout(() => {
    __privateSet(this, _fadeTimer2, void 0);
    if (__privateGet(this, _phase) === GlowPhase.FADING)
      __privateSet(this, _phase, GlowPhase.IDLE);
  }, __privateGet(this, _glowFadeMs)));
};
...
var _paceMs, _root6, _glow3, _cursor2, _ring, _hud, _actLine, _actStrip, _chatPillText, _chatPillTime, _chip, _tally, _tallied, _mode, _now4, _heartbeatMs, _idleNoticeMs, _borderMode2, _glowCtl, _lastActionText, _heartbeatTimer, _idleEndMs, _sessionId2, _startMs, _endMs, _runLog, _sessionActive, _logMax, _log, _logBaseMs, _onControl, _panel3, _shell, _annotator, _Presenter_instances, syncAnnotator_fn, showSession_fn, revive_fn, paintActStrip_fn, startHeartbeat_fn, tickLiveness_fn, endIdle_fn, clearRunLog_fn, applyHideUntilRestart_fn, onSettingsChange_fn, renderTally_fn, pruneLog_fn;
var Presenter = class {
  constructor(options = {}) {
    __privateAdd(this, _Presenter_instances);
    __privateAdd(this, _paceMs);
    __privateAdd(this, _root6);
    __privateAdd(this, _glow3);
    __privateAdd(this, _cursor2);
    __privateAdd(this, _ring);
    __privateAdd(this, _hud);
    __privateAdd(this, _actLine);
    __privateAdd(this, _actStrip);
    /** Label inside the minimised-chat capsule; mirrors the act strip. */
    __privateAdd(this, _chatPillText);
    /** Age of the last action, shown in the capsule's own slot. */
    __privateAdd(this, _chatPillTime);
    __privateAdd(this, _chip);
    /** Live verdict tally (✓N ✗M) in the header - the running testing score the human watches. */
    __privateAdd(this, _tally);
    __privateAdd(this, _tallied, { passes: 0, fails: 0 });
    __privateAdd(this, _mode, PresenterMode.IDLE);
    __privateAdd(this, _now4);
    __privateAdd(this, _heartbeatMs);
    __privateAdd(this, _idleNoticeMs);
    __privateAdd(this, _borderMode2);
    /** The glow / activity state machine (border shimmer + cursor visibility from activity timing). */
    __privateAdd(this, _glowCtl);
    /** Liveness: the most recent action text + a 1s ticker that ages it into an "idle · {dur}" clock. */
    __privateAdd(this, _lastActionText, "");
    __privateAdd(this, _heartbeatTimer);
    /** Session lifecycle: idle-end window (tweakable), session id, start/end cursors, structured run log. */
    __privateAdd(this, _idleEndMs);
    __privateAdd(this, _sessionId2);
    __privateAdd(this, _startMs);
    __privateAdd(this, _endMs);
    __privateAdd(this, _runLog, []);
    /** Tracks sessionStart/sessionEnd so both are idempotent (no strobe / no spurious off-write). */
    __privateAdd(this, _sessionActive, false);
    // v2: narration + action status accumulate in a persistent, timestamped, scrollable log.
    __privateAdd(this, _logMax);
    __privateAdd(this, _log);
    /** now of the first row, the baseline for the +elapsed timestamps. */
    __privateAdd(this, _logBaseMs);
    // Live-control panel: the two-way control surface (Pause/Resume + End + message Send).
    __privateAdd(this, _onControl);
    __privateAdd(this, _panel3);
    __privateAdd(this, _shell);
    __privateAdd(this, _annotator);
    __privateSet(this, _paceMs, effectivePaceMs(options.paceMs));
    __privateSet(this, _now4, options.now ?? nativeNow);
    __privateSet(this, _heartbeatMs, options.heartbeatMs ?? HEARTBEAT_MS);
    __privateSet(this, _idleNoticeMs, options.idleNoticeMs ?? IDLE_NOTICE_MS);
    __privateSet(this, _idleEndMs, options.idleEndMs ?? IDLE_END_MS);
    __privateSet(this, _sessionId2, options.sessionId ?? "");
    __privateSet(this, _borderMode2, options.border ?? DEFAULT_BORDER_MODE);
    __privateSet(this, _glowCtl, new GlowController({
      now: __privateGet(this, _now4),
      idleAfterMs: options.idleAfterMs ?? IDLE_AFTER_MS,
      glowFadeMs: options.glowFadeMs ?? GLOW_FADE_MS,
      borderMode: __privateGet(this, _borderMode2),
      setMode: (mode) => this.setMode(mode)
    }));
    __privateSet(this, _logMax, clampLogMax(options.logMax));
    __privateSet(this, _onControl, options.onControl);
    __privateSet(this, _panel3, new ControlPanel({
      emit: (kind, text) => {
        var _a3;
        return (_a3 = __privateGet(this, _onControl)) == null ? void 0 : _a3.call(this, text !== void 0 ? { kind, text } : { kind });
      },
      logHuman: (text) => {
      ...
      clearRunLog: () => __privateMethod(this, _Presenter_instances, clearRunLog_fn).call(this),
      onStateChange: () => __privateMethod(this, _Presenter_instances, syncAnnotator_fn).call(this)
    }));
    __privateSet(this, _shell, new HudShell({
      onChatOpen: () => __privateGet(this, _shell).pulseFab(false),
      onExpand: () => __privateMethod(this, _Presenter_instances, syncAnnotator_fn).call(this),
      onAnnotateToggle: () => __privateMethod(this, _Presenter_instances, syncAnnotator_fn).call(this),
      onCollapse: () => __privateMethod(this, _Presenter_instances, syncAnnotator_fn).call(this),
      settings: {
        onBeforeOpen: () => {
          if (__privateGet(this, _shell).isCollapsed())
            __privateGet(this, _shell).expand();
        },
        onHideUntilRestart: () => __privateMethod(this, _Presenter_instances, applyHideUntilRestart_fn).call(this),
        onSettingsChange: (s) => __privateMethod(this, _Presenter_instances, onSettingsChange_fn).call(this, s)
      }
    }));
  }
  /** Setter so reticle.ts can wire the control callback after construction. */
  setControlHandler(handler) {
    __privateSet(this, _onControl, handler);
  }
  /** Current live-control session state mirrored onto the panel (data-reticle-state). */
  get state() {
    return __privateGet(this, _panel3).state;
  }
  /** Whether a run is currently being presented (false before the agent's first activity / after end). */
  get sessionActive() {
    return __privateGet(this, _sessionActive);
  }
  /** Drive the panel's live-control visual state (server-push / agent path; never emits). */
  setState(state, text, tone) {
    __privateGet(this, _panel3).setState(state, text, tone);
  }
  ...
  refilterFlows() {
    __privateGet(this, _panel3).refilterFlows();
  }
  handlePush(command) {
    const a = command.args;
    if (command.name === ReticleCommand.FLOWS)
      return void __privateGet(this, _panel3).setFlows(a["flows"]);
    if (command.name === ReticleCommand.IMPACT) {
      const snapshot = parseImpactSnapshot(a["snapshot"]);
      if (snapshot !== void 0)
        __privateGet(this, _shell).report.setSnapshot(snapshot);
      return;
    }
    const state = a["state"];
    const tone = a["tone"];
  ...
  get logMax() {
    return __privateGet(this, _logMax);
  }
  set logMax(n) {
    __privateSet(this, _logMax, clampLogMax(n));
    __privateMethod(this, _Presenter_instances, pruneLog_fn).call(this);
  }
  mount() {
    if (__privateGet(this, _root6) !== void 0 || "undefined" === typeof document)
      return;
    const style = document.createElement("style");
    ...
    __privateSet(this, _glow3, root.querySelector("[data-reticle-glow]") ?? void 0);
    __privateSet(this, _cursor2, root.querySelector("[data-reticle-cursor]") ?? void 0);
    __privateSet(this, _ring, root.querySelector("[data-reticle-ring]") ?? void 0);
    __privateSet(this, _hud, root.querySelector("[data-reticle-hud]") ?? void 0);
    __privateSet(this, _actLine, root.querySelector(".reticle-act") ?? void 0);
    __privateSet(this, _actStrip, root.querySelector(".reticle-act-strip") ?? void 0);
    __privateSet(this, _chatPillText, root.querySelector("[data-reticle-chat-pill-text]") ?? void 0);
    __privateSet(this, _chatPillTime, root.querySelector("[data-reticle-chat-pill-time]") ?? void 0);
    __privateSet(this, _log, root.querySelector(`[${DATA_RETICLE_LOG}]`) ?? void 0);
    __privateSet(this, _chip, root.querySelector("[data-reticle-chip]") ?? void 0);
    __privateSet(this, _tally, root.querySelector("[data-reticle-tally]") ?? void 0);
    __privateGet(this, _shell).mount(root);
    syncPageBlocker(root, getPresenterSettings(), false);
    __privateGet(this, _glowCtl).setElements(__privateGet(this, _glow3), __privateGet(this, _cursor2));
    __privateGet(this, _panel3).mount(root, __privateGet(this, _glow3));
    this.setMode(__privateGet(this, _mode));
    __privateMethod(this, _Presenter_instances, renderTally_fn).call(this);
  }
  /** Wire annotation chrome; expanding the HUD enters annotate mode. */
  bindAnnotator(annotator) {
    __privateSet(this, _annotator, annotator);
    const root = __privateGet(this, _root6);
    if (root === void 0)
      return;
    ...
    __privateMethod(this, _Presenter_instances, syncAnnotator_fn).call(this);
  }
  destroy() {
    var _a3;
    __privateGet(this, _glowCtl).teardown();
    if (__privateGet(this, _heartbeatTimer) !== void 0)
      nativeClearTimeout(__privateGet(this, _heartbeatTimer));
    __privateSet(this, _heartbeatTimer, void 0);
    __privateGet(this, _shell).teardown();
    __privateGet(this, _panel3).teardown();
    __privateSet(this, _sessionActive, false);
    __privateSet(this, _logBaseMs, void 0);
    __privateSet(this, _log, void 0);
    (_a3 = __privateGet(this, _root6)) == null ? void 0 : _a3.remove();
    document.querySelectorAll("style[data-reticle-overlay]").forEach((s) => s.remove());
    __privateSet(this, _root6, void 0);
  }
  /**
  ...
  sessionStart() {
    if (this.state === SessionState.ENDED) {
      __privateMethod(this, _Presenter_instances, revive_fn).call(this);
      return;
    }
    if (__privateGet(this, _sessionActive))
      return;
    __privateSet(this, _sessionActive, true);
    __privateGet(this, _startMs) ?? __privateSet(this, _startMs, __privateGet(this, _now4).call(this));
    __privateSet(this, _endMs, void 0);
    __privateMethod(this, _Presenter_instances, showSession_fn).call(this);
    __privateGet(this, _glowCtl).resetActivity(__privateGet(this, _now4).call(this));
    __privateMethod(this, _Presenter_instances, startHeartbeat_fn).call(this);
    if (getPresenterSettings().autoOpenChat) {
      __privateGet(this, _shell).openChat();
    }
  }
  /**
  ...
  sessionEnd() {
    ...
    const dock = (_a3 = __privateGet(this, _root6)) == null ? void 0 : _a3.querySelector("[data-reticle-dock]");
    dock == null ? void 0 : dock.setAttribute(DATA_ON, GLOW_OFF);
    (_b = __privateGet(this, _hud)) == null ? void 0 : _b.setAttribute(DATA_ON, GLOW_OFF);
    __privateGet(this, _shell).collapse();
    if (__privateGet(this, _borderMode2) === BorderMode.SESSION) {
      (_c = __privateGet(this, _glow3)) == null ? void 0 : _c.setAttribute(DATA_ON, GLOW_OFF);
      (_d = __privateGet(this, _glow3)) == null ? void 0 : _d.setAttribute(DATA_BUSY, BUSY_OFF);
    }
  }
  /**
   ...
   */
  markActivity() {
    __privateGet(this, _glowCtl).markActivity();
  }
  /** Re-arm the quiet-window idle check (kept for reticle.ts's finally block). */
  scheduleIdle() {
    __privateGet(this, _glowCtl).scheduleIdle();
  }
  /** Test/diagnostic accessor for the current glow phase. */
  glowPhase() {
    return __privateGet(this, _glowCtl).phase();
  }
  ...
  setMode(mode) {
    ...
    (_a3 = __privateGet(this, _root6)) == null ? void 0 : _a3.setAttribute("data-reticle-mode", mode);
    if (__privateGet(this, _chip) !== void 0) {
      const label = CHIP_LABEL[mode];
      __privateGet(this, _chip).setAttribute("data-mode", mode);
      __privateGet(this, _chip).replaceChildren();
      if (label.length > 0) {
        const icon = mode === PresenterMode.READING ? PresenterIcon.VIEW : mode === PresenterMode.ACTING ? PresenterIcon.POINTER : void 0;
        if (icon !== void 0)
          __privateGet(this, _chip).appendChild(hiIcon(icon, PRESENTER_ICON_SIZE.CHIP));
        const labelEl = document.createElement("span");
        labelEl.className = "reticle-chip-label";
        labelEl.textContent = label;
        __privateGet(this, _chip).appendChild(labelEl);
      }
    }
    if (mode === PresenterMode.READING)
      (_b = __privateGet(this, _cursor2)) == null ? void 0 : _b.setAttribute(DATA_ON, GLOW_OFF);
  }
  status(text) {
    this.markActivity();
    if (__privateGet(this, _chatPillTime) !== void 0)
      __privateGet(this, _chatPillTime).textContent = ACT_STRIP.NOW;
    __privateSet(this, _lastActionText, text);
    __privateMethod(this, _Presenter_instances, paintActStrip_fn).call(this, text, false);
  }
  /** Agent-tunable idle-end window (reticle_session). Floored so it can't be set uselessly small. */
  setIdleEndMs(ms) {
    if (!Number.isFinite(ms))
      return;
    __privateSet(this, _idleEndMs, Math.max(IDLE_END_MIN_MS, Math.floor(ms)));
  }
  /**
  ...
  runState() {
    const base = buildRunState({
      ...
      state: this.state,
      startMs: __privateGet(this, _startMs),
      endMs: __privateGet(this, _endMs),
      now: __privateGet(this, _now4).call(this),
      runLog: __privateGet(this, _runLog)
    });
  ...
  log(kind, text, result2) {
    const ms = __privateGet(this, _now4).call(this);
    __privateGet(this, _glowCtl).markActivity(ms);
    if (__privateGet(this, _log) === void 0)
      return void 0;
    const trimmed = text.trim();
    if (0 === trimmed.length)
      return void 0;
    __privateGet(this, _logBaseMs) ?? __privateSet(this, _logBaseMs, ms);
    const entry = result2 !== void 0 ? { at: ms - __privateGet(this, _logBaseMs), kind, text: trimmed, result: result2 } : { at: ms - __privateGet(this, _logBaseMs), kind, text: trimmed };
    __privateGet(this, _runLog).push(entry);
    while (__privateGet(this, _runLog).length > __privateGet(this, _logMax))
      __privateGet(this, _runLog).shift();
    const ts = formatElapsed(ms - __privateGet(this, _logBaseMs));
    const handle = appendLogRow(__privateGet(this, _log), kind, trimmed, ts, __privateGet(this, _logMax));
    if (result2 !== void 0)
      handle.result(result2);
    if (__privateGet(this, _shell).isCollapsed())
      __privateGet(this, _shell).pulseFab(true);
    __privateMethod(this, _Presenter_instances, renderTally_fn).call(this);
    return {
      result: (r) => {
        handle.result(r);
  ...
  setThrottled(throttled) {
    var _a3;
    (_a3 = __privateGet(this, _root6)) == null ? void 0 : _a3.setAttribute(THROTTLED_ATTR, throttled ? "1" : "0");
    if (throttled && __privateGet(this, _actLine) !== void 0) {
      __privateGet(this, _actLine).textContent = "Tab backgrounded - actions throttled. Bring tab to front or use `reticle drive`.";
    }
  }
  ...
  async beforeAct(refId, action, label) {
    ...
    if (!(el instanceof HTMLElement)) {
      await pace(__privateGet(this, _paceMs));
      return;
    }
    const rect = el.getBoundingClientRect();
    const cx = rect.left + rect.width / 2;
    const cy = rect.top + rect.height / 2;
    moveCursor(__privateGet(this, _cursor2), cx, cy);
    ringAround(__privateGet(this, _ring), rect);
    await pace(__privateGet(this, _paceMs));
    if (ActionType.CLICK === action || ActionType.DBLCLICK === action || ActionType.SUBMIT === action)
      spawnRipple(__privateGet(this, _root6), cx, cy);
  }
};
_paceMs = new WeakMap();
_root6 = new WeakMap();
...
_ring = new WeakMap();
_hud = new WeakMap();
_actLine = new WeakMap();
_actStrip = new WeakMap();
_chatPillText = new WeakMap();
_chatPillTime = new WeakMap();
_chip = new WeakMap();
_tally = new WeakMap();
_tallied = new WeakMap();
_mode = new WeakMap();
_now4 = new WeakMap();
_heartbeatMs = new WeakMap();
_idleNoticeMs = new WeakMap();
_borderMode2 = new WeakMap();
_glowCtl = new WeakMap();
_lastActionText = new WeakMap();
_heartbeatTimer = new WeakMap();
_idleEndMs = new WeakMap();
_sessionId2 = new WeakMap();
_startMs = new WeakMap();
_endMs = new WeakMap();
_runLog = new WeakMap();
_sessionActive = new WeakMap();
_logMax = new WeakMap();
_log = new WeakMap();
_logBaseMs = new WeakMap();
_onControl = new WeakMap();
_panel3 = new WeakMap();
_shell = new WeakMap();
_annotator = new WeakMap();
_Presenter_instances = new WeakSet();
/** Annotate whenever the HUD is open and the person asked for it - agent or no agent. */
syncAnnotator_fn = function() {
  var _a3;
  const live = !__privateGet(this, _shell).isCollapsed() && __privateGet(this, _shell).isAnnotateOn();
  (_a3 = __privateGet(this, _annotator)) == null ? void 0 : _a3.toggle(live);
  if (__privateGet(this, _root6) !== void 0) {
    syncPageBlocker(__privateGet(this, _root6), getPresenterSettings(), live);
  }
};
/** Turn the base border (session mode) + the HUD/log on - the visible "session is live" state. */
showSession_fn = function() {
  var _a3, _b, _c;
  const dock = (_a3 = __privateGet(this, _root6)) == null ? void 0 : _a3.querySelector("[data-reticle-dock]");
  dock == null ? void 0 : dock.setAttribute(DATA_ON, GLOW_ON);
  (_b = __privateGet(this, _hud)) == null ? void 0 : _b.setAttribute(DATA_ON, GLOW_ON);
  if (__privateGet(this, _borderMode2) === BorderMode.SESSION)
    (_c = __privateGet(this, _glow3)) == null ? void 0 : _c.setAttribute(DATA_ON, GLOW_ON);
};
/** Revive after an ended session (new agent activity): clear the ended state + glow back on. */
revive_fn = function() {
  __privateGet(this, _panel3).setState(SessionState.ACTIVE);
  __privateSet(this, _endMs, void 0);
  __privateMethod(this, _Presenter_instances, showSession_fn).call(this);
  __privateGet(this, _glowCtl).resetActivity(__privateGet(this, _now4).call(this));
  __privateMethod(this, _Presenter_instances, startHeartbeat_fn).call(this);
 ...
 */
paintActStrip_fn = function(text, idle) {
  var _a3;
  if (__privateGet(this, _actLine) !== void 0)
    __privateGet(this, _actLine).textContent = text;
  const liveness = idle ? "idle" : "active";
  if (__privateGet(this, _actStrip) !== void 0)
    __privateGet(this, _actStrip).setAttribute("data-liveness", liveness);
  (_a3 = __privateGet(this, _root6)) == null ? void 0 : _a3.setAttribute(LIVENESS_ATTR, liveness);
  if (__privateGet(this, _chatPillText) !== void 0) {
    __privateGet(this, _chatPillText).textContent = idle ? __privateGet(this, _lastActionText) !== "" ? __privateGet(this, _lastActionText) : ACT_STRIP.READY : text;
  }
};
/**
...
startHeartbeat_fn = function() {
  if (__privateGet(this, _heartbeatTimer) !== void 0)
    nativeClearTimeout(__privateGet(this, _heartbeatTimer));
  const tick = () => {
    __privateMethod(this, _Presenter_instances, tickLiveness_fn).call(this);
    __privateSet(this, _heartbeatTimer, nativeSetTimeout(tick, __privateGet(this, _heartbeatMs)));
  };
  __privateSet(this, _heartbeatTimer, nativeSetTimeout(tick, __privateGet(this, _heartbeatMs)));
};
tickLiveness_fn = function() {
  if (!__privateGet(this, _sessionActive) || __privateGet(this, _actLine) === void 0)
    return;
  if (this.state === SessionState.ENDED)
    return;
  const idleMs = __privateGet(this, _now4).call(this) - __privateGet(this, _glowCtl).lastActivityMs();
  if (idleMs >= __privateGet(this, _idleEndMs)) {
    __privateMethod(this, _Presenter_instances, endIdle_fn).call(this, idleMs);
    return;
  }
  if (idleMs < __privateGet(this, _idleNoticeMs))
    return;
  const since = __privateGet(this, _lastActionText) !== "" ? ACT_STRIP.SINCE_LAST : "";
  __privateMethod(this, _Presenter_instances, paintActStrip_fn).call(this, `${ACT_STRIP.IDLE_PREFIX}${humanDuration(idleMs)}${since}`, true);
  if (__privateGet(this, _chatPillTime) !== void 0)
    __privateGet(this, _chatPillTime).textContent = humanDuration(idleMs);
};
/** Auto-end after the idle window: stamp the end, drive the panel to ENDED, stop the heartbeat. */
endIdle_fn = function(idleMs) {
  __privateSet(this, _endMs, __privateGet(this, _now4).call(this));
  __privateGet(this, _panel3).setState(SessionState.ENDED, `idle ${humanDuration(idleMs)}`);
  if (__privateGet(this, _heartbeatTimer) !== void 0) {
    nativeClearTimeout(__privateGet(this, _heartbeatTimer));
    __privateSet(this, _heartbeatTimer, void 0);
  }
};
clearRunLog_fn = function() {
  __privateSet(this, _runLog, []);
  __privateSet(this, _tallied, { passes: 0, fails: 0 });
  if (__privateGet(this, _log) !== void 0)
    __privateGet(this, _log).replaceChildren();
  __privateMethod(this, _Presenter_instances, renderTally_fn).call(this);
};
applyHideUntilRestart_fn = function() {
  var _a3;
  (_a3 = __privateGet(this, _root6)) == null ? void 0 : _a3.setAttribute("data-reticle-hidden", "1");
  __privateGet(this, _shell).collapse();
};
onSettingsChange_fn = function(settings) {
  var _a3, _b;
  (_a3 = __privateGet(this, _annotator)) == null ? void 0 : _a3.setAccent(statusTheme(settings.statusThemeId).active);
  if (!settings.showTally) {
    (_b = __privateGet(this, _tally)) == null ? void 0 : _b.setAttribute("hidden", "");
  } else {
    __privateMethod(this, _Presenter_instances, renderTally_fn).call(this);
  }
  if (__privateGet(this, _root6) !== void 0) {
    const live = SessionState.ACTIVE === __privateGet(this, _panel3).state && !__privateGet(this, _shell).isCollapsed();
    syncPageBlocker(__privateGet(this, _root6), settings, live);
  }
};
/** Repaint the header verdict tally from the run log; the side that grew gets a one-shot pop. */
renderTally_fn = function() {
  var _a3;
  if (!getPresenterSettings().showTally) {
    (_a3 = __privateGet(this, _tally)) == null ? void 0 : _a3.setAttribute("hidden", "");
    return;
  }
  __privateSet(this, _tallied, renderTally(__privateGet(this, _tally), __privateGet(this, _runLog), __privateGet(this, _tallied)));
};
pruneLog_fn = function() {
  var _a3;
  if (__privateGet(this, _log) === void 0)
    return;
  while (__privateGet(this, _log).childElementCount > __privateGet(this, _logMax)) {
    (_a3 = __privateGet(this, _log).firstElementChild) == null ? void 0 : _a3.remove();
  }
};
...
function anchorFor(el) {
  const testid = el.getAttribute(TESTID_ATTR3);
  if (testid !== null && testid.length > 0) {
...
var _deps2, _phase2, _steps, _startPath, _annotations, _pendingFill, _teardowns, _root7, _statusEl, _menuEl, _draft, _Recorder_instances, installCapture_fn, ignore_fn, onClick_fn, onInput_fn, onChange_fn, onSubmit_fn, flushPendingFill_fn, captureAnnotationTarget_fn, confirmAnnotationOnPrior_fn, selectedSignal_fn, start_fn, stop_fn, openAnnotateMenu_fn, setPhase_fn, buildToolbar_fn, button_fn, renderMenu_fn, chooseKind_fn, closeMenu_fn, setStatus_fn, nameField_fn;
var Recorder = class {
  constructor(deps) {
    __privateAdd(this, _Recorder_instances);
    __privateAdd(this, _deps2);
    __privateAdd(this, _phase2, RecorderPhase.IDLE);
    __privateAdd(this, _steps, []);
    /** The pathname the recording began on, so replay can navigate here before step 1. */
    __privateAdd(this, _startPath);
    __privateAdd(this, _annotations, []);
    __privateAdd(this, _pendingFill);
    __privateAdd(this, _teardowns, []);
    __privateAdd(this, _root7);
    __privateAdd(this, _statusEl);
    __privateAdd(this, _menuEl);
    /** The annotation being assembled in ANNOTATING phase. */
    __privateAdd(this, _draft);
    __privateSet(this, _deps2, deps);
  }
  phase() {
    return __privateGet(this, _phase2);
  }
  steps() {
    return __privateGet(this, _steps).map((s) => ({ ...s }));
  }
  mount() {
    if (__privateGet(this, _root7) !== void 0)
      return;
    if ("undefined" === typeof document)
      return;
    __privateMethod(this, _Recorder_instances, buildToolbar_fn).call(this);
    __privateMethod(this, _Recorder_instances, installCapture_fn).call(this);
  }
  destroy() {
    var _a3;
    for (const t of __privateGet(this, _teardowns))
      t();
    __privateSet(this, _teardowns, []);
    (_a3 = __privateGet(this, _root7)) == null ? void 0 : _a3.remove();
    __privateSet(this, _root7, void 0);
    __privateSet(this, _statusEl, void 0);
    __privateSet(this, _menuEl, void 0);
    __privateSet(this, _phase2, RecorderPhase.IDLE);
    __privateSet(this, _steps, []);
    __privateSet(this, _annotations, []);
    __privateSet(this, _draft, void 0);
    __privateSet(this, _pendingFill, void 0);
  }
};
_deps2 = new WeakMap();
_phase2 = new WeakMap();
_steps = new WeakMap();
_startPath = new WeakMap();
_annotations = new WeakMap();
_pendingFill = new WeakMap();
_teardowns = new WeakMap();
_root7 = new WeakMap();
_statusEl = new WeakMap();
_menuEl = new WeakMap();
_draft = new WeakMap();
_Recorder_instances = new WeakSet();
// ---- capture ----
installCapture_fn = function() {
  const onClick = (ev) => __privateMethod(this, _Recorder_instances, onClick_fn).call(this, ev);
  const onInput = (ev) => __privateMethod(this, _Recorder_instances, onInput_fn).call(this, ev);
  const onChange = (ev) => __privateMethod(this, _Recorder_instances, onChange_fn).call(this, ev);
  const onSubmit = (ev) => __privateMethod(this, _Recorder_instances, onSubmit_fn).call(this, ev);
  document.addEventListener("click", onClick, true);
  document.addEventListener("input", onInput, true);
  document.addEventListener("change", onChange, true);
  document.addEventListener("submit", onSubmit, true);
  __privateGet(this, _teardowns).push(() => {
    document.removeEventListener("click", onClick, true);
...
onClick_fn = function(ev) {
  const target = __privateMethod(this, _Recorder_instances, ignore_fn).call(this, ev);
  if (target === void 0)
    return;
  if (__privateGet(this, _phase2) === RecorderPhase.ANNOTATING) {
    __privateMethod(this, _Recorder_instances, captureAnnotationTarget_fn).call(this, target);
    return;
  }
  if (__privateGet(this, _phase2) !== RecorderPhase.RECORDING)
    return;
  ...
  __privateMethod(this, _Recorder_instances, flushPendingFill_fn).call(this);
  __privateGet(this, _steps).push(buildStep(target, ActionType.CLICK));
};
onInput_fn = function(ev) {
  if (__privateGet(this, _phase2) !== RecorderPhase.RECORDING)
    return;
  const target = __privateMethod(this, _Recorder_instances, ignore_fn).call(this, ev);
  if (target === void 0 || !isTextbox(target))
    return;
  __privateSet(this, _pendingFill, { el: target, value: target.value });
};
onChange_fn = function(ev) {
  if (__privateGet(this, _phase2) !== RecorderPhase.RECORDING)
    return;
  const target = __privateMethod(this, _Recorder_instances, ignore_fn).call(this, ev);
  if (target === void 0)
  ...
  if (target instanceof HTMLInputElement && "checkbox" === inputRole2(target)) {
    __privateMethod(this, _Recorder_instances, flushPendingFill_fn).call(this);
    __privateGet(this, _steps).push(buildStep(target, target.checked ? ActionType.CHECK : ActionType.UNCHECK));
    return;
  }
  if (target instanceof HTMLInputElement && "radio" === inputRole2(target)) {
    __privateMethod(this, _Recorder_instances, flushPendingFill_fn).call(this);
    __privateGet(this, _steps).push(buildStep(target, ActionType.CHECK));
    return;
  }
  if (target instanceof HTMLSelectElement) {
    __privateMethod(this, _Recorder_instances, flushPendingFill_fn).call(this);
    __privateGet(this, _steps).push(buildStep(target, ActionType.SELECT, { value: target.value }));
    return;
  }
  if (isTextbox(target)) {
    __privateSet(this, _pendingFill, { el: target, value: target.value });
...
onSubmit_fn = function(ev) {
  ...
  if (target === void 0)
    return;
  __privateMethod(this, _Recorder_instances, flushPendingFill_fn).call(this);
  __privateGet(this, _steps).push(buildStep(target, ActionType.SUBMIT));
};
flushPendingFill_fn = function() {
  const pending = __privateGet(this, _pendingFill);
  if (pending === void 0)
    return;
  __privateSet(this, _pendingFill, void 0);
  const value = isSensitiveField(pending.el) ? REDACTED_VALUE : pending.value;
  __privateGet(this, _steps).push(buildStep(pending.el, ActionType.FILL, { value }));
};
// ---- annotation ----
captureAnnotationTarget_fn = function(target) {
  const draft = __privateGet(this, _draft);
  if (draft === void 0)
    return;
  const { anchor } = anchorFor(target);
  const ann = { kind: draft.kind, anchor };
  const signal = __privateMethod(this, _Recorder_instances, selectedSignal_fn).call(this);
  if (signal !== void 0)
    ann.signal = signal;
  __privateGet(this, _annotations).push(ann);
  __privateSet(this, _draft, void 0);
  __privateMethod(this, _Recorder_instances, setPhase_fn).call(this, RecorderPhase.RECORDING);
  __privateMethod(this, _Recorder_instances, closeMenu_fn).call(this);
};
/** Annotate-on-prior: confirm the most-recent step as the target (no extra click needed). */
confirmAnnotationOnPrior_fn = function() {
  const draft = __privateGet(this, _draft);
  if (draft === void 0)
    return;
  const last = __privateGet(this, _steps).at(-1);
  if (last === void 0) {
    __privateSet(this, _draft, void 0);
    __privateMethod(this, _Recorder_instances, setPhase_fn).call(this, RecorderPhase.RECORDING);
    __privateMethod(this, _Recorder_instances, closeMenu_fn).call(this);
...
selectedSignal_fn = function() {
  var _a3;
  const select = (_a3 = __privateGet(this, _menuEl)) == null ? void 0 : _a3.querySelector("[data-reticle-signal]");
  const value = (select == null ? void 0 : select.value) ?? "";
  return value.length > 0 ? value : void 0;
};
// ---- toolbar lifecycle actions ----
start_fn = function() {
  __privateSet(this, _steps, []);
  __privateSet(this, _startPath, "undefined" === typeof location ? void 0 : location.pathname);
  __privateSet(this, _annotations, []);
  __privateSet(this, _pendingFill, void 0);
  __privateSet(this, _draft, void 0);
  __privateMethod(this, _Recorder_instances, setPhase_fn).call(this, RecorderPhase.RECORDING);
  __privateMethod(this, _Recorder_instances, setStatus_fn).call(this, STATUS_RECORDING);
};
stop_fn = function() {
  __privateMethod(this, _Recorder_instances, flushPendingFill_fn).call(this);
  const name = __privateMethod(this, _Recorder_instances, nameField_fn).call(this) ?? __privateGet(this, _deps2).defaultName ?? DEFAULT_NAME;
  const flow = compileRecording(name, __privateGet(this, _steps), __privateGet(this, _annotations), __privateGet(this, _deps2).now(), __privateGet(this, _startPath));
  __privateGet(this, _deps2).emit(EventType.FLOW_RECORDED, { name, flow });
  __privateMethod(this, _Recorder_instances, setStatus_fn).call(this, 0 === __privateGet(this, _steps).length ? RECORDER_EMPTY_MSG : STATUS_IDLE);
  __privateMethod(this, _Recorder_instances, setPhase_fn).call(this, RecorderPhase.IDLE);
  __privateMethod(this, _Recorder_instances, closeMenu_fn).call(this);
};
openAnnotateMenu_fn = function() {
  if (__privateGet(this, _phase2) !== RecorderPhase.RECORDING)
    return;
  __privateMethod(this, _Recorder_instances, setPhase_fn).call(this, RecorderPhase.ANNOTATING);
  __privateMethod(this, _Recorder_instances, setStatus_fn).call(this, STATUS_ANNOTATE);
  __privateMethod(this, _Recorder_instances, renderMenu_fn).call(this);
};
setPhase_fn = function(phase) {
  __privateSet(this, _phase2, phase);
};
// ---- toolbar DOM (all nodes data-reticle-overlay → snapshot-excluded via dom-ignore.ts) ----
buildToolbar_fn = function() {
  const root = document.createElement("div");
  root.setAttribute("data-reticle-overlay", "");
  ...
  root.appendChild(__privateMethod(this, _Recorder_instances, button_fn).call(this, "record", BUTTON_LABEL.record, () => __privateMethod(this, _Recorder_instances, start_fn).call(this)));
  root.appendChild(__privateMethod(this, _Recorder_instances, button_fn).call(this, "stop", BUTTON_LABEL.stop, () => __privateMethod(this, _Recorder_instances, stop_fn).call(this)));
  root.appendChild(__privateMethod(this, _Recorder_instances, button_fn).call(this, "annotate", BUTTON_LABEL.annotate, () => __privateMethod(this, _Recorder_instances, openAnnotateMenu_fn).call(this)));
  ...
  __privateSet(this, _statusEl, status);
  __privateSet(this, _menuEl, menu);
};
button_fn = function(action, label, onClick) {
  const btn = document.createElement("button");
...
renderMenu_fn = function() {
  const menu = __privateGet(this, _menuEl);
  if (menu === void 0)
    return;
  menu.textContent = "";
  for (const kind of Object.values(AnnotationKind)) {
    ...
    item.addEventListener("click", () => __privateMethod(this, _Recorder_instances, chooseKind_fn).call(this, kind));
    menu.appendChild(item);
  }
};
chooseKind_fn = function(kind) {
  __privateSet(this, _draft, { kind });
  const menu = __privateGet(this, _menuEl);
  if (menu === void 0)
    return;
  ...
  confirm.addEventListener("click", () => __privateMethod(this, _Recorder_instances, confirmAnnotationOnPrior_fn).call(this));
  menu.appendChild(confirm);
};
closeMenu_fn = function() {
  if (__privateGet(this, _menuEl) !== void 0)
    __privateGet(this, _menuEl).textContent = "";
};
setStatus_fn = function(text) {
  if (__privateGet(this, _statusEl) !== void 0)
    __privateGet(this, _statusEl).textContent = text;
};
nameField_fn = function() {
  var _a3;
  const input = (_a3 = __privateGet(this, _root7)) == null ? void 0 : _a3.querySelector("[data-reticle-name]");
...
function connectionPolicy(pageHostname, bridgeUrl, allowNonLocalhost, token, pageProtocol = "http:") {
  ...
  if (((token == null ? void 0 : token.length) ?? 0) > TRANSPORT_LIMITS.MAX_TOKEN_LENGTH) {
    return {
      allowed: false,
...
var _transport, _registry, _teardowns2, _connected, _session, _documentId, _start, _overlay, _presenter, _recorder, _annotator2, _eventCount, _token, _sdkVersion, _projectId, _redactKeys, _actHandle, _emit2, _Reticle_instances, applyRedaction_fn, hello_fn, handleCommand_fn, presentBefore_fn;
var Reticle = class {
  constructor() {
    __privateAdd(this, _Reticle_instances);
    __privateAdd(this, _transport);
    __privateAdd(this, _registry, /* @__PURE__ */ new Map());
    __privateAdd(this, _teardowns2, []);
    __privateAdd(this, _connected, false);
    __privateAdd(this, _session, "default");
    /**
     ...
     */
    __privateAdd(this, _documentId, newDocumentId(Math.random));
    __privateAdd(this, _start, 0);
    __privateAdd(this, _overlay);
    __privateAdd(this, _presenter);
    __privateAdd(this, _recorder);
    __privateAdd(this, _annotator2);
    __privateAdd(this, _eventCount, 0);
    __privateAdd(this, _token);
    __privateAdd(this, _sdkVersion);
    __privateAdd(this, _projectId);
    /** App-declared extra redaction keys, announced in hello so the driven path honours them too. */
    __privateAdd(this, _redactKeys, []);
    /** Act-row log handle for the in-flight act/act_sequence, so its outcome stamps the right row. */
    __privateAdd(this, _actHandle);
    __privateAdd(this, _emit2, (type, data, ref) => {
      var _a3, _b, _c;
      const event = buildEvent({
        seq: __privateGet(this, _eventCount),
        t: Math.round(performance.now() - __privateGet(this, _start)),
        type,
        sessionId: __privateGet(this, _session),
        documentId: __privateGet(this, _documentId),
        editEpoch: editEpoch.current,
        data,
        ref
      });
      try {
        (_a3 = __privateGet(this, _transport)) == null ? void 0 : _a3.sendEvent(event);
        __privateSet(this, _eventCount, __privateGet(this, _eventCount) + 1);
        (_b = __privateGet(this, _overlay)) == null ? void 0 : _b.update({ connected: true, events: __privateGet(this, _eventCount) });
        if (type === EventType.ROUTE_CHANGE)
          (_c = __privateGet(this, _presenter)) == null ? void 0 : _c.refilterFlows();
      } catch {
      }
    });
  }
  connect(options = {}) {
    ...
    const nodeEnv = (_a3 = proc == null ? void 0 : proc.env) == null ? void 0 : _a3["NODE_ENV"];
    ...
    __privateSet(this, _sdkVersion, "string" === typeof declaredVersion && declaredVersion.length > 0 ? declaredVersion : void 0);
    ...
    __privateSet(this, _session, rememberSessionLabel(explicitSession.length > 0 ? explicitSession : void 0, "undefined" === typeof globalThis.sessionStorage ? void 0 : globalThis.sessionStorage, () => {
      var _a4;
      return "function" === typeof ((_a4 = globalThis.crypto) == null ? void 0 : _a4.randomUUID) ? `s${globalThis.crypto.randomUUID()}` : `s${Date.now().toString(36)}`;
    }));
    __privateSet(this, _token, options.token !== void 0 && options.token.length > 0 ? options.token : void 0);
    __privateSet(this, _projectId, identity.projectId);
    __privateMethod(this, _Reticle_instances, applyRedaction_fn).call(this, options.redact);
    __privateSet(this, _start, performance.now());
    __privateSet(this, _registry, createCommandRegistry());
    __privateSet(this, _transport, new Transport({
      url,
      hello: () => __privateMethod(this, _Reticle_instances, hello_fn).call(this),
      handleCommand: (command) => __privateMethod(this, _Reticle_instances, handleCommand_fn).call(this, command),
      ...
      onConnected: () => {
        var _a4;
        return (_a4 = __privateGet(this, _presenter)) == null ? void 0 : _a4.sessionStart();
      },
      ...
      onConnectionLost: () => {
        var _a4;
        resetClock();
        if (true === ((_a4 = __privateGet(this, _presenter)) == null ? void 0 : _a4.sessionActive)) {
          __privateGet(this, _presenter).setState(SessionState.ENDED, BRIDGE_LOST_SUMMARY);
        }
      },
    ...
    setCapabilitiesListener(() => {
      var _a4;
      return (_a4 = __privateGet(this, _transport)) == null ? void 0 : _a4.reannounce();
    });
    const emit = __privateGet(this, _emit2);
    __privateSet(this, _teardowns2, installAllObservers(emit, {
      captureBodies: true === options.captureNetworkBodies
    }));
    if (true === options.overlay) {
      __privateSet(this, _overlay, installOverlay());
      __privateGet(this, _overlay).update({ connected: true, events: 0 });
    }
    if (options.present !== false) {
      ...
      presenterOptions.sessionId = __privateGet(this, _session);
      presenterOptions.onControl = (intent) => __privateGet(this, _emit2).call(this, EventType.HUMAN_CONTROL, intent.text !== void 0 ? { kind: intent.kind, text: intent.text } : { kind: intent.kind });
      __privateSet(this, _presenter, new Presenter(presenterOptions));
      __privateGet(this, _presenter).mount();
    }
    if (true === options.recorder) {
      __privateSet(this, _recorder, installRecorder({ emit, now: () => Date.now() }));
      __privateGet(this, _recorder).mount();
    }
    if (options.annotate ?? options.present !== false) {
      const presenter = __privateGet(this, _presenter);
      __privateSet(this, _annotator2, new Annotator({
        emit,
        now: () => Date.now(),
        onMark: (mark) => presenter == null ? void 0 : presenter.log(LOG_KIND.HUMAN, `🚩 #${String(mark.index)} ${mark.anchor}${mark.source !== void 0 ? ` · ${mark.source}` : ""} — ${mark.note}`),
        shouldBlock: () => getPresenterSettings().blockPageInteractions
      }));
      __privateGet(this, _annotator2).mount();
      (_b = __privateGet(this, _presenter)) == null ? void 0 : _b.bindAnnotator(__privateGet(this, _annotator2));
    }
    __privateGet(this, _transport).connect();
  ...
  signal(name, data = {}) {
    __privateGet(this, _emit2).call(this, EventType.SIGNAL, { name, data });
  }
  /** Report a framework/store state change the agent can observe and assert on. */
  state(name, value) {
    __privateGet(this, _emit2).call(this, EventType.STATE_CHANGE, { name, value });
  }
  /**
  ...
  renderCommit(commits) {
    if (commits > 0)
      __privateGet(this, _emit2).call(this, EventType.RENDER_COMMIT, { commits });
  }
  ...
  endSession() {
    var _a3;
    (_a3 = __privateGet(this, _presenter)) == null ? void 0 : _a3.setState(SessionState.ENDED);
  }
  /**
  ...
  disconnect() {
    ...
    if (!__privateGet(this, _connected))
      return;
    for (const teardown of __privateGet(this, _teardowns2))
      teardown();
    __privateSet(this, _teardowns2, []);
    (_a3 = __privateGet(this, _transport)) == null ? void 0 : _a3.close();
    __privateSet(this, _transport, void 0);
    (_b = __privateGet(this, _overlay)) == null ? void 0 : _b.destroy();
    __privateSet(this, _overlay, void 0);
    (_c = __privateGet(this, _presenter)) == null ? void 0 : _c.sessionEnd();
    (_d = __privateGet(this, _presenter)) == null ? void 0 : _d.destroy();
    __privateSet(this, _presenter, void 0);
    (_e = __privateGet(this, _recorder)) == null ? void 0 : _e.destroy();
    __privateSet(this, _recorder, void 0);
    (_f = __privateGet(this, _annotator2)) == null ? void 0 : _f.destroy();
    __privateSet(this, _annotator2, void 0);
    resetClock();
...
_teardowns2 = new WeakMap();
_connected = new WeakMap();
_session = new WeakMap();
_documentId = new WeakMap();
_start = new WeakMap();
_overlay = new WeakMap();
_presenter = new WeakMap();
_recorder = new WeakMap();
_annotator2 = new WeakMap();
_eventCount = new WeakMap();
_token = new WeakMap();
_sdkVersion = new WeakMap();
_projectId = new WeakMap();
_redactKeys = new WeakMap();
_actHandle = new WeakMap();
_emit2 = new WeakMap();
_Reticle_instances = new WeakSet();
/**
 ...
 * previous one set.
 */
applyRedaction_fn = function(config) {
  setActiveRedactionPolicy(buildRedactionPolicy(config, nativeWarn));
  __privateSet(this, _redactKeys, wireRedactionKeys(config));
};
hello_fn = function() {
  return {
    ...
    sessionId: __privateGet(this, _session),
    ...__privateGet(this, _projectId) === void 0 ? {} : { projectId: __privateGet(this, _projectId) },
    url: location.href,
    title: document.title,
    adapters: adapterNames(),
    ...__privateGet(this, _token) === void 0 ? {} : { token: __privateGet(this, _token) },
    hasCapabilities: hasCapabilities(),
    // Absent when no build plugin supplied one - "unknown", never "matching".
    ...__privateGet(this, _sdkVersion) === void 0 ? {} : { sdkVersion: __privateGet(this, _sdkVersion) },
    ...
    ...0 === __privateGet(this, _redactKeys).length ? {} : { redactKeys: __privateGet(this, _redactKeys) }
  };
};
handleCommand_fn = async function(command) {
  var _a3, _b, _c, _d, _e, _f, _g, _h;
  if (command.name === ReticleCommand.NARRATE) {
    (_a3 = __privateGet(this, _presenter)) == null ? void 0 : _a3.sessionStart();
    (_b = __privateGet(this, _presenter)) == null ? void 0 : _b.narrate(str2(command.args["text"]), str2(command.args["level"], "info"));
    return { ok: true, result: { shown: __privateGet(this, _presenter) !== void 0 } };
  }
  if (command.name === ReticleCommand.SESSION_CONFIG) {
    const idleEndMs = command.args["idleEndMs"];
    if ("number" === typeof idleEndMs)
      (_c = __privateGet(this, _presenter)) == null ? void 0 : _c.setIdleEndMs(idleEndMs);
    return { ok: true, result: { applied: __privateGet(this, _presenter) !== void 0, idleEndMs } };
  }
  if (command.name === ReticleCommand.PRESENTER || command.name === ReticleCommand.FLOWS || command.name === ReticleCommand.IMPACT) {
    (_d = __privateGet(this, _presenter)) == null ? void 0 : _d.handlePush(command);
    return { ok: true, result: { applied: __privateGet(this, _presenter) !== void 0 } };
  }
  const handler = __privateGet(this, _registry).get(command.name);
  if (handler === void 0) {
    return { ok: false, error: `unknown command '${command.name}'` };
  }
  (_e = __privateGet(this, _presenter)) == null ? void 0 : _e.sessionStart();
  await __privateMethod(this, _Reticle_instances, presentBefore_fn).call(this, command);
  try {
    const result2 = await handler(command.args);
    (_f = __privateGet(this, _actHandle)) == null ? void 0 : _f.result(LOG_RESULT.PASS);
    return { ok: true, result: result2 };
  } catch (error) {
    (_g = __privateGet(this, _actHandle)) == null ? void 0 : _g.result(LOG_RESULT.FAIL);
    return { ok: false, error: error instanceof Error ? error.message : String(error) };
  } finally {
    __privateSet(this, _actHandle, void 0);
    (_h = __privateGet(this, _presenter)) == null ? void 0 : _h.scheduleIdle();
  }
};
presentBefore_fn = async function(command) {
  const p = __privateGet(this, _presenter);
  if (p === void 0)
    return;
  ...
  if (command.name === ReticleCommand.ACT) {
    ...
    __privateSet(this, _actHandle, p.log(LOG_KIND.ACT, `${actionVerb(str2(command.args["action"]))} ${label}`));
    ...
    for (const step of steps) {
      ...
      const label = refLabel(ref);
      __privateSet(this, _actHandle, p.log(LOG_KIND.ACT, `${actionVerb(str2(s.action))} ${label}`));
    ...
    getState: () => {
      ...
      for (const query of client.getQueryCache().getAll()) {
        ...
        out[key] = {
          ...
          isStale: (_a3 = query.isStale) == null ? void 0 : _a3.call(query),
          dataUpdatedAt: query.state.dataUpdatedAt,
          error: ((_b = query.state.error) == null ? void 0 : _b.message) ?? null,
...
var reticle = globalStore4.__reticleInstance ?? (globalStore4.__reticleInstance = new Reticle());
...
//# sourceMappingURL=chunk-OLFSJKW4.js.map
