...
import { IntentState } from './intent.js';
/**
...
function isCurrent(evidence, currentDocumentId, currentEditEpoch) {
    return (isSameDocument(evidence.doc, currentDocumentId) &&
 ...
 */
function foldScoped(existing, incoming, keyOf, currentDocumentId, currentEditEpoch) {
    const current = (row) => isCurrent(row, currentDocumentId, currentEditEpoch);
...
export function foldEstablished(existing, incoming, currentDocumentId, currentEditEpoch) {
...
export function foldProven(existing, incoming, currentDocumentId, currentEditEpoch) {
 ...
 */
export function remainingFor(intents) {
...
export function buildRunContext(input) {
    return {
        step: input.step,
        established: foldEstablished([], input.established, input.currentDocumentId, input.currentEditEpoch),
