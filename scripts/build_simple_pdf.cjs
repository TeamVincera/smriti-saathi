...
async function run() {
  const mdPath = path.resolve(__dirname, '../SIH_PROJECT_DETAILS_SIMPLE.md');
  const pdfPath = path.resolve(__dirname, '../Smriti_Sathi_Simple_Guide.pdf');
  const tempHtml = path.resolve(__dirname, '../temp_view.html');
  ...
  <head>
    <meta charset="utf-8">
    <title>Smriti Sathi — Simple Technical Guide</title>
  ...
  <body>
    <div class="banner">
      ...
      <h1>Smriti Sathi (স্মৃতি সাথী • स्मृति साथी)</h1>
      <p>A Simple Language Guide to the App's Technical Architecture, Math, Features & PPT Talking Points</p>
    </div>
    ${newLines.join('\n')}
  </body>
  </html>`;
