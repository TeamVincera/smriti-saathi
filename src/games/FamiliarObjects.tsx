...
import type { FamiliarObjectItem } from '../lib/questionBanks/familiarObjects'

const TOTAL_ROUNDS = 5
const DOMAIN = 'recognition'

export function FamiliarObjects({ difficulty, logAction, complete }: GameProps) {
  const { lang } = useApp()
  const rng = useRef(sessionRng('objects')).current
  const [roundIdx, setRoundIdx] = useState(0)
  const [level, setLevel] = useState(() => Math.max(difficulty, nextLevel(DOMAIN)))

  const round = useMemo(() => genFamiliarObjectRound(rng, level), [rng, level, roundIdx])
  ...
  function pick(item: FamiliarObjectItem) {
    if (pickedId) return
    setPickedId(item.id)
    ...
    <div className="center-col" style={{ width: '100%', maxWidth: 880, margin: '0 auto' }}>
      ...
      <div className="card card-dark enter-anim" style={{ width: '100%', padding: 'var(--s-md) var(--s-lg)', marginBottom: 'var(--s-lg)' }}>
        <span className="caption" style={{ color: 'var(--muga-gold-light)', display: 'block', marginBottom: 4 }}>
          🏡 Everyday Household & Craft Objects
        </span>
